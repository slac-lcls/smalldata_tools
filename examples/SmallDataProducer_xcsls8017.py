...
def getROI(run):
    ...
    if run <=1:
        ...
        return [ [[366,218], [566,418]] ]
    elif run>=35 and run<107:
	return [ [[189,562], [580,698]] ]
    elif run>=107 and run<166:
	return [ [[365,634], [473,611]] ]
    elif run>=166 and run<191:
	return [ [[341,556], [209,380]] ]
    elif run>=191 and run<270:
	return [ [[406,661], [422,566]] ]
    elif run>=270 :
	return [ [[339,556], [236,343]] ]
    else:
	return [ [[366,218], [566,418]] ]
...
if have_epix:
    ...
    for iROI,roi in enumerate(ROI):
        epix.addROI('ROI_%d'%iROI, roi, writeArea=True)
        epix.addPhotons(ADU_per_photon=165, thresADU=0.85, retImg=2, nphotMax=200, ROI=roi)
    dets.append(epix)
