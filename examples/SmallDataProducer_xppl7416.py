...
def getROI_cspad(run):
    if isinstance(run,basestring):
        run=int(run)
#    return [ [[3,4],   [26,175], [249,376]],
#             [[11,12], [32,149], [248,356]],
#             [[0,1], [1,74], [312,381]],
...
if checkDet(ds.env(), 'epix_Rowland'):
    rl = DetObject('epix_Rowland' ,ds.env(), int(run), common_mode=46)
    for iROI,ROI in enumerate(ROI_rowland):
        rl.addROI('ROI%d'%iROI, ROI)
#    cs140.ROI.addProj('_p0',axis=0,cutADU=15)
    #dets.append(rl)
...
if haveCspad:
    ...
    #    try:
    #        cspad.addAzAv(phiBins=7, Pplane=0)
    #    except:
    #        pass
    #dets.append(cspad)
