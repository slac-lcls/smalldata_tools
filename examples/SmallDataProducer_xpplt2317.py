...
def getNmaxDrop(run):
    ...
    if run >= 100:
    	return 50 # used 1000 up to run 87
    else:
    	return 500
...
#epicsPV = ['slit_s1_hw'] 
epicsPV = ['ath', 'atth', 'achi', 'anaz', 'detz', 'samr', 'huberhead_tilt1', 'huberhead_tilt2', 'huberhead_tx', 'huberhead_ty', 'samx', 'samy', 'Cryojet_SampleTemp', 'Cryojet_SampleTempSet']
...
if have_epix:
    ...
    epix = DetObject(epixname ,ds.env(), int(run), name=epixname,common_mode=4)
    
    ##this return an image in image form. takes ROI=[] as parameter.
