...
def getAzIntParams(run):
    ...
    ret_dict = {'eBeam': 7.7}
    ret_dict['cspad_center'] = [89999.519564, 93244.941941]
    ret_dict['cspad_dis_to_sam'] = 65.
    return ret_dict
...
for iepix,epixname in enumerate(epixnames):
    ...
    if have_epix:
        ...
        #epix['droplet'].add_aduHist([50.,600.,1.])
        #same, but with ROI
        ...
        #epix46 = DetObject(epixname ,ds.env(), int(run), common_mode=46, name='epix_1_46')
        #epix46.addDroplet(threshold=10., thresholdLow=3., thresADU=0.,name='droplet')
        ##save a histogram of ADU values in detector/event
        #if not args.skipDrop:
        #    epix46['droplet'].addDropletSave(maxDroplets=nDrop)
        #dets.append(epix46)

        #epixpedSub = DetObject(epixname ,ds.env(), int(run), common_mode=1, name='epix_1_pedSub')
        #epixpedSub.addDroplet(threshold=10., thresholdLow=3., thresADU=0.,name='droplet')
        ##save a histogram of ADU values in detector/event
        #if not args.skipDrop:
        #    epixpedSub['droplet'].addDropletSave(maxDroplets=nDrop)
        #dets.append(epixpedSub)
