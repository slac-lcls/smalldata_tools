...
def getROI_sum(run):
    ...
    if run < 22:
        ...
        return [ [[0,1], [218,494], [14,1008]] ]
    else:
        return [ [[0,1], [101,490], [245,594]] ]
...
if haveJungfrau512k:
    ...
    for iROI,ROI in enumerate(ROI_sum):
        jungfrau512k.addROI('ROIsum_%d'%iROI, ROI)#,writeArea=True)
        #sum with threshold.
        jungfrau512k['ROIsum_%d'%iROI].addProj(axis=-1, thresADU=7.9)
