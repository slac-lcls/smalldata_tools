...
ROI_opal = getROI_opal(int(run))
if checkDet(ds.env(), 'opal_2'):
    opal = DetObject('opal_2' ,ds.env(), int(run))
    for iROI,ROI in enumerate(ROI_opal):
        opal.addROI('ROI_%d'%iROI, ROI)
    if len(ROI_opal)>0:
        opal.ROI_0.addProj('0', axis=0)
        opal.ROI_0.addProj('1', axis=1)
    dets.append(opal)

ROI_jungfrau = getROI_jungfrau(int(run))
haveJungfrau = checkDet(ds.env(), 'jungfrau1M')
if haveJungfrau:
    jungfrau = DetObject('jungfrau1M' ,ds.env(), int(run))
