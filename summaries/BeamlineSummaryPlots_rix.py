...
def makeRunTableData(dat):
    ...
    runtable_data = {"N dropped Shots":int(dropped.sum()),
                     "N total":int(total),
...
######################################
dat = tables.open_file('/reg/d/psdm/%s/%s/hdf5/smalldata/%s_Run%04d.h5'%(expname[:3],expname,expname,run)).root
...
fim0 = -1.*dat.det_rix_fim0.full_sigbkg_sum.read()[:,4:].sum(axis=1)
fim1 = -1.*dat.det_rix_fim1.full_sigbkg_sum.read()[:,5]
...
i0Med =  [np.nanmedian(i0Var[i*120:i*120+120]) for i in range(int(eventTimeR.shape[0]/120))]

#this should be the andor when present
#scatterVar = dat.gmd.avgIntensity.read()
try: 
    scatterVar = dat.andor_dir.ROI_sig_sum.read()-dat.andor_dir.ROI_bkg_sum.read()/550*400
except:
    scatterVar = None

i0Time = hv.HexTiles((eventTimeR[i0Var<np.nanpercentile(i0Var,99)],
                         i0Var[i0Var<np.nanpercentile(i0Var,99)]),
                        kdims=[eventTimeDim, i0Dim]).opts(cmap='Blues')
i0TimeMed = hv.Points((eventTimeRMed, i0Med), kdims=[eventTimeDim,i0Dim],label=i0Dim.label).options(color='r')
...
try:
    ...
    andorPlot = hv.Curve((np.arange(andor_trace.shape[0]),andor_trace), kdims=[pixelDim],vdims=[andorDim])
except:
    andorPlot = None
...
try:
    ...
    if eventTimeR[on].shape[0]<5000:
        ttposPlot = hv.Scatter((eventTimeR[on], ttpos[on]), kdims=[eventTimeDim, ttposDim])
        ...
        ttposPlot = hv.HexTiles((eventTimeR[on], ttpos[on]), kdims=[eventTimeDim, ttposDim])
        ttamplPlot = hv.HexTiles((ttampl[on], i0Var[on]), kdims=[ttamplDim, i0Dim])
        ttamplposPlot = hv.HexTiles((ttpos[on], ttampl[on]), kdims=[ttposDim, ttamplDim])
except:
    ttpos = None

fimDims = {'ch%d'%i: hv.Dimension(('fim_ch%d'%i,'channel %d'%i)) for i in np.arange(dat.det_rix_fim0.full_area.shape[1])}
fimDims['legend'] = hv.Dimension(('fim_legend','legend'))
fimTimeDim = hv.Dimension(('fim_time','fim points'))
fimsums = {'fim%d'%ifim: getattr(dat,'det_rix_fim%d'%ifim).full_area.read().sum(axis=0) for ifim in np.arange(3)}


fimCurves = {'channel_%d'%i: hv.NdOverlay({fn: hv.Curve((np.arange(256),fimsums[fn][i]), kdims=[fimTimeDim],vdims=[fimDims['ch%d'%i]]).opts(show_legend=False) for fn in fimsums }) for i in np.arange(fimsums['fim0'].shape[0])}
fimCurves['legend'] = hv.NdOverlay({fn: hv.Curve((np.arange(256),np.zeros(256)), kdims=[fimTimeDim],vdims=[fimDims['legend']]).opts(hooks=[click_policy]) for fn in fimsums })

gspecFim = pn.GridSpec(sizing_mode='stretch_both', max_width=700, name='Summed FIM traces - Run %d'%run)
for k,plot in fimCurves.items():
    try:
        ...
        irow=2
        icol=2
    gspecFim[irow*3:(irow+1)*3, icol*3:(icol+1)*3] = pn.Column(plot.opts(legend_position='bottom_right'))
...
gspecS = pn.GridSpec(sizing_mode='stretch_both', max_width=700, name='Andor, Scan&Scatter')
#gspec[0,0:8] = pn.Row('# Data Quality Plot - Run %04d'%run)
maxRow=0
