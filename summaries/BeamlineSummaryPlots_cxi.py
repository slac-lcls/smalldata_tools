...
try:
    ...
    j4mPlot = hv.Curve((np.arange(j4m_qbins),j4m_trace), kdims=[pixelDim],vdims=[j4mDim])
except:
    j4mPlot = None
...
try:
    ...
    if eventTimeR[on].shape[0]<5000:
        ...
        ttfwhmposPlot = hv.HexTiles((ttpos[on], ttfwhm[on]), kdims=[ttposDim, ttfwhmDim])
except:
    ttpos = None
...
if nOff>100:
    ...
    for detImgName in ana.Keys('Sums'):
        ...
        detGrid = pn.GridSpec(sizing_mode='fixed', max_width=700, name='%s, dropped shots'%detName)
        detGrid[0,0] = pn.Row(detImgs[-1])
        detGrids.append(detGrid)       
...
gspecS = pn.GridSpec(sizing_mode='stretch_both', max_width=700, name='jungfrau4M, Scan&Scatter')
#gspec[0,0:8] = pn.Row('# Data Quality Plot - Run %04d'%run)
maxRow=0
