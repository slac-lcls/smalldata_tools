...
def rebin(a, shape):
    ...
    if (a.shape[0] % shape[0]) == 0 and (a.shape[1] % shape[1]) == 0:
        ...
        factor = [
            float(int(a.shape[0] / shape[0]) + 1),
            float(int(a.shape[1] / shape[1]) + 1),
        ]
        bigImg = ndimage.zoom(
            a, [shape[0] * factor[0] / a.shape[0], shape[1] * factor[1] / a.shape[1]]
        )
...
def getBins(bindef=[], debug=False):
    ...
    if isinstance(bindef, np.ndarray):
        bindef = bindef.tolist()
    if len(bindef) > 3:
    ...
    if len(bindef) == 3:
        if type(bindef[2]) is int:
            Bins = np.linspace(
                min(bindef[0], bindef[1]),
                max(bindef[0], bindef[1]),
                bindef[2] + 1,
                endpoint=True,
            )
        else:
            Bins = np.arange(
                min(bindef[0], bindef[1]), max(bindef[0], bindef[1]), bindef[2]
            )
    ...
    if len(bindef) == 2:
        if debug:
            print(
                "only two bin boundaries, so this is effectively a cut...cube will have a single image"
            )
...
def getDelay(fh5, use_ttCorr=True, addEnc=False):
    ...
    for node in fh5.get_node("/scan")._f_list_nodes():
        if (
            node.name.find("var") < 0
            and node.name.find("none") < 0
            and node.name.find("lxt") >= 0
            and node.name.find("damage") < 0
        ):
            isDaqDelayScan = True
...
_ = get_offVar_nomean(
    np.arange(5), np.ones(5).astype(int), np.array([0, 0, 1, 1, 2]), nNbr=1
)
...
def cm_epix(
    ...
    if colrow % 2 == 1:
        ...
        rsmed[rscount > ((1.0 - minFrac) * 352)] = 0
        imgCorr = np.ma.masked_array(
            (rs.data - rsmed[None, :]).data.reshape(704, 768, order="F"), imgThres
        )
    else:
    ...
    if colrow >= 2:
        ...
        rsmed[abs(rsmed) > maxCorr] = 0
        rsmed[rscount > ((1.0 - minFrac) * 96)] = 0
        imgCorr = (np.ma.masked_array(rs.data - rsmed[:, None], imgThres)).reshape(
            704, 768
        )
...
def cm_uxi(dataFrame, cm_photonThres, cm_maxCorr, cm_minFrac, cm_maskNeighbors):
    ...
    for frame in maskedImg:
        ...
        rsmed[rscount > ((1.0 - cm_minFrac) * frame.shape[1])] = (
            0  # do not correct if too few pixels contribute
        )
        # print('rsmed3 ',(rsmed.data==0).sum())
        cm_RowMedsApplied.append(rsmed.data.tolist())
        imgCorr = (rs.data - rsmed[:, None]).reshape(
            maskedImg[0].shape[0], maskedImg[0].shape[1], order="F"
        )
...
def templateArray(args, template, nPeaks, templateShape):
    ...
    for i in range(nPeaks):
        ...
        if (templateShape - templatePk.shape[0]) > 0:
            templatePk = np.append(
                templatePk, np.zeros(templateShape - templatePk.shape[0])
            )
        ...
        templatep = templatePk * (1.0 - frac1) + templatePkp * frac1
        ##        if args[3]==0:
...
def hist2d(
    ...
    limits=[1, 99.5],
    numBins=[100, 100],
    histLims=[np.nan, np.nan, np.nan, np.nan],
    weights=None,
    doPlot=True,
):
    pmin0 = np.nanmin(ar1)
    ...
    if doPlot:
        plt.imshow(
            ...
            clim=[np.percentile(iSig, limits[0]), np.percentile(iSig, limits[1])],
        )
        plt.colorbar()
    return iSig
...
#
def fitTrace(data, kind="stepUp", weights=None, iterFrac=-1.0, maxPeak=100):
    ...
    try:
        below = (f < ((f[-25:].mean() + tamp) / 2.0)).nonzero()[0] - mpr
        tfwhm = np.abs(np.min(below[below > 0]) - np.max(below[below < 0]))
        retDict["below"] = below
    except:
        tfwhm = 0.0
    retDict["pos"] = tpos + nWeights / 2.0
    retDict["amp"] = tamp
    ...
    if iterFrac > 0.0:
        ...
        while np.max(fP) > iterFrac * allAmp[0]:
            ...
            # now do a parabolic fit around the max
            xd = np.arange(
                max(0, mprIter - halfrange), min(mprIter + halfrange, len(fP) - 1)
            )
            ...
            tamp = np.polyval(p2, tpos)
            try:
                below = (fP < ((fP[-25:].mean() + tamp) / 2.0)).nonzero()[0] - mprIter
                try:
                ...
                allPos.append(tpos + nWeights / 2.0)
                allAmp.append(tamp)
                if np.isnan(tamp):
...
def findPeakSimple(trace, nMaxPeak=2, peakWid=20, peakOff=5):
    ...
    maxIdx = [
        x
        for _, x in sorted(
            zip(trace[peakind], peakIdx), key=lambda pair: pair[0], reverse=True
        )
    ]
    peakIdxS = []
    peakIdxV = []
    ...
    for peak in maxIdx:
        ...
        peakIdxSum.append(
            trace[peak - peakWid / 2 + peakOff : peak + peakWid / 2 + peakOff].sum()
        )
...
def shapeFromKey_h5(fh5, thiskey):
    if len(thiskey.split("/")) > 2:
        return fh5.get_node(
            "/".join(thiskey.split("/")[:-1]), thiskey.split("/")[-1]
        ).shape
    else:
...
def rename_reduceRandomVar(outFileName):
    ...
    for k in fin.keys():
        if isinstance(fin[k], h5py.Group):
            continue
        if k.find("Cfg") >= 0 or k == "cubeSelection" or k.find("dim") >= 0:
            continue
        ...
        if len(fin[k].shape) >= len(nEntryShape):
            ...
            print(
                f"Cannot determine shape for reshaping {k}: {fin[k].shape} to {nEntryShape}"
            )
...
def getCMpeak(img, nPeakSel=4, minPeakNum=100, ADUmin=-100, ADUmax=200, step=0.5):
    ...
    maxIdx = [
        x
        for _, x in sorted(
            zip(his[0][peakIdx], peakIdx), key=lambda pair: pair[0], reverse=True
        )
    ]
    maxIdxFilter = [
        x
        for x in maxIdx
        if his[0][max(0, x - 2) : min(x + 3, his[0].shape[0])].sum() > minPeakNum
    ]
...
def image_from_dxy(d, x, y, **kwargs):
    if np.array(x).shape != np.array(y).shape or np.array(d).shape != np.array(y).shape:
        print(
            "shapes of data or x/y do not match ",
            np.array(d).shape,
            np.array(x).shape,
            np.array(y).shape,
        )
    ...
    if (
        np.abs(x.flatten()[0] - int(x.flatten()[0])) < 1e-12
        ...
        # cspad
        if x.shape == (32, 185, 388):
            imgShape = [1689, 1689]
        # cs140k
        elif x.shape == (2, 185, 388):
            imgShape = [391, 371]  # at least for one geometry
        # epix100a
        elif x.shape == (704, 768):
            imgShape = [709, 773]
        # jungfrau512k
        elif x.shape == (1, 512, 1024):
            imgShape = [514, 1030]
        elif x.shape == (512, 1024):
            imgShape = [514, 1030]
        # jungfrau1M
        elif x.shape == (2, 512, 1024):
            imgShape = [1064, 1030]
            ...
            if pixelSize is None:
                ...
                imgShape = [
                    (x.max() - x.min()) / pixelSize,
                    (y.max() - y.min()) / pixelSize,
                ]
        else:
            if outShape is None:
                print(
                    "do not know which detector in need of a special geometry this is, cannot determine shape of image"
                )
            else:
                imgShape = outShape
            return
    ...
    img = np.asarray(
        sparse.coo_matrix(
            (d.flatten(), (ix.flatten(), iy.flatten())), shape=outShape
        ).todense()
    )
    return img
...
def unsparsify(data, shape):
    ...
    if len(shape) == 2:
        dropdata = sparse.coo_matrix(
            (data["data"], ((data["row"]).astype(int), (data["col"]).astype(int))),
            shape=shape,
        ).toarray()
        return dropdata
    if len(shape) > 2:
...
def KdeCuts(values, bandwidth="scott", percentile=[0.1, 99.9], nBins=1000):
    ...
    xValues = np.linspace(
        np.percentile(values, percentile[0]),
        np.percentile(values, percentile[1]),
        nBins,
    )
    ...
    kde_left_dist = [
        dist_to_segment(retDict["firstPt"], retDict["maxPt"], xpt, ypt)
        for xpt, ypt in zip(xValues[:kde_xMaxIdx], kde_yValues[:kde_xMaxIdx])
    ]
    leftMaxDistIdx = np.argmax(kde_left_dist)
    leftMaxDistX = xValues[leftMaxDistIdx]
    leftMaxDistVal = kde_yValues[leftMaxDistIdx]
    leftMaxDistLineVal = retDict["firstPt"][1] + (
        retDict["maxPt"][1] - retDict["firstPt"][1]
    ) / (retDict["maxPt"][0] - retDict["firstPt"][0]) * (
        leftMaxDistX - retDict["firstPt"][0]
    )
    retDict["leftMaxDist"] = [
        leftMaxDistX,
        leftMaxDistLineVal,
        leftMaxDistVal,
        np.max(kde_left_dist),
    ]
    # distance to lines - right
    kde_right_dist = [
        dist_to_segment(retDict["maxPt"], retDict["lastPt"], xpt, ypt)
        for xpt, ypt in zip(xValues[kde_xMaxIdx:], kde_yValues[kde_xMaxIdx:])
    ]
    ...
    mRight = (retDict["lastPt"][1] - retDict["maxPt"][1]) / (
        retDict["lastPt"][0] - retDict["maxPt"][0]
    )
    rightMaxDistLineVal = retDict["maxPt"][1] + mRight * (
        rightMaxDistX - retDict["maxPt"][0]
    )
    retDict["rightMaxDist"] = [
        rightMaxDistX,
        rightMaxDistLineVal,
        rightMaxDistVal,
        np.max(kde_right_dist),
    ]
