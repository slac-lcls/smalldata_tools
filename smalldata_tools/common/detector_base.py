    ...
    def params_as_dict(self):
        ...
        parList =  {
            key:self.__dict__[key] for key in self.__dict__ if (
                key[0]!='_' and isinstance(getattr(self,key), (str, int, float, np.ndarray, tuple))
            ) 
        }
        parList.update(
            {
                key: np.array(self.__dict__[key]) for key in self.__dict__ if (
                    key[0]!='_' and isinstance(getattr(self,key), list) \
...
class DetObjectFunc(object):
    ...
    def params_as_dict(self):
        ...
        parList =  {
            key: self.__dict__[key] for key in self.__dict__ if (
                isinstance(getattr(self,key), (str, int, float, np.ndarray)) and key[0]!='_'
            )
        }
        parList.update({
            key: np.array(self.__dict__[key]) for key in self.__dict__ if (
                isinstance(getattr(self,key), list) and key[0]!='_'
            )
        })
