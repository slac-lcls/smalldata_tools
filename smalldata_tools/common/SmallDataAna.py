...
class Cube(object):
    ...
    def __init__(self, binVar, bins=[], useFilter=None, cubeName=None, addBinVars=None):
        ...
        if nbins == 3:
            if type(bins[2]) is int:
                nbins = len(
                    np.linspace(min(bins[0], bins[1]), max(bins[0], bins[1]), bins[2])
                )
            else:
                nbins = len(
                    np.arange(min(bins[0], bins[1]), max(bins[0], bins[1]), bins[2])
                )
        ...
        self.binBounds = (
            None  # bin boundary array (could be same as bins, but does not need to)
        )
    ...
    def add_BinVar(self, addBinVars):
        ...
        if isinstance(addBinVars, list):
            ...
            for addVar in addBinVars.keys():
                ...
                if addbins is None:
                    print(
                        "Could not define bin boundaries for addBinVars:",
                        addBinVars[addVar],
                    )
                    print("automatic detection only works for primary binning axis")
    ...
    def printCube(self, Sel=None):
        ...
        if len(self.bins) == 3 and type(self.bins[2]) is int:
            print(
                "%s binned from %g to %g in %i bins"
                % (self.binVar, self.bins[0], self.bins[1], self.bins[2])
            )
            ...
            print(
                ...
                % (
                    ...
                    self.bins[0],
                    self.bins[1],
                    int((self.bins[1] - self.bins[0]) / self.bins[2]),
                )
            )
            ...
            print(
                "%s binned from %g to %g in %i bins"
                % (self.binVar, min(self.bins), max(self.bins), len(self.bins))
            )
        if Sel is not None:
...
class Selection(object):
    ...
    def printCuts(self):
        ...
        if isinstance(self._filter, np.ndarray):
            print(
                "of %d events %d pass filter"
                % (self._filter.shape[0], self._filter.sum())
            )
...
class SmallDataAna(object):
    ...
    def __init__(
        ...
        if len(expname) > 3:
            ...
            self.plot_dirname = (
                f"{S3DF_BASE}/{self.hutch}/{self.expname}/results/smalldata_plots/"
            )
            if dirname == "":
                if hostname.find("drp-srcf") >= 0:
                    self.dirname = (
                        f"{FFB_BASE}/{self.hutch}/{self.expname}/scratch/hdf5/smalldata"
                    )
                elif self.onS3DF:
                    self.dirname = (
                        f"{S3DF_BASE}/{self.hutch}/{self.expname}/hdf5/smalldata"
                    )
                    self.plot_dirname = f"{S3DF_BASE}/{self.hutch}/{self.expname}/results/smalldata_plots/"
                else:
                    self.dirname = "/reg/d/psdm/%s/%s/hdf5/smalldata" % (
                        self.hutch,
                        self.expname,
                    )
                ...
                if not path.isdir(self.dirname):
                    ...
                    self.plot_dirname = "/reg/d/psdm/%s/%s/res/smalldata_plots/" % (
                        self.hutch,
                        self.expname,
                    )
            else:
        ...
        if filename == "":
            ...
            if not self.onS3DF and not path.isdir(
                "/reg/d/psdm/%s/%s/results/" % (self.hutch, self.expname)
            ):
                self.fname = "%s/ldat_%s_Run%03d.h5" % (
                    self.dirname,
                    self.expname,
                    self.run,
                )
        ...
        if intable is not None:
            if isinstance(intable, str) and path.isfile(intable):
                ...
                print(
                    "Pass, unknown input parameter or file cannot be found: ", intable
                )
                return None
        ...
        try:
            if "event_time" in self._fields.keys():
                ...
                if timeData is None:
                    ...
                    self._tStamp = np.array(
                        [
                            np.datetime64(int(ttime[0] * 1e9 + ttime[1]), "ns")
                            for ttime in timeData
                        ]
                    )
                    self.xrData = xr.DataArray(
                        evttime,
        ...
        self.bokehpalette = [
            pltm.colors.rgb2hex(m) for m in colormap(np.arange(colormap.N))
        ]
        ...
        for node in self.fh5.root._f_list_nodes():
            ...
            if not isinstance(node, tables.group.Group):
                ...
                if self._fields[fieldkey][1] == "onDisk":
                    try:
                        this_node_data = xr.DataArray(
                            self.fh5.get_node(key).read(),
                            coords={"time": self._tStamp},
                            dims=("time"),
                            name=key[1:].replace("/", "__"),
                        )
                        self.xrData = xr.merge([self.xrData, this_node_data])
                        self._fields[fieldkey][1] = "inXr"
                    except:
                        print(
                            "failed to create dataset for: ",
                            fieldkey,
                            self._fields[fieldkey],
                        )
                continue
            for tleaf in node._f_list_nodes():
                if not isinstance(tleaf, tables.group.Group):
                    ...
                    if (
                        self.fh5.get_node(key, tleaf.name).shape[0]
                        != self._tStamp.shape[0]
                    ):
                        continue
                    ...
                    if len(dataShape) <= 2 and coords is not None:
                        if (
                            len(dataShape) == 2
                            and dataShape[1] > 25
                            and (dataShape[0] * dataShape[1]) > 1e7
                        ):
                            print(
                                f"Array for {fieldName} is too large (shape {dataShape}), will load when asked."
                            )
                            continue
                        this_node_data = xr.DataArray(
                            self.fh5.get_node(key, tleaf.name).read().squeeze(),
                            coords=coords,
                            dims=dims,
                            name=tArrName,
                        )
    ...
    def addVar(self, name="newVar", data=[]):
        ...
        if name.find("__") >= 0 and name.replace("__", "/") not in self._fields.keys():
            print(
                "Names of newly defined variables may not contain __, will replace with single _"
            )
        ...
        data = data.squeeze()
        if len(data.shape) == 1:
            newArray = xr.DataArray(
                data, coords={"time": self._tStamp}, dims=("time"), name=name
            )
        ...
        for key in self._fields.keys():
            ...
            if self._fields[key][2] == "mem":
                ...
                if isinstance(data, xr.DataArray):
                    ...
                    if len(data.shape) == 1:
                        new_xrData = xr.DataArray(
                            data, coords={"time": self._tStamp}, dims=("time"), name=key
                        )
                    else:
                        coords = {"time": self._tStamp}
                        dims = ["time"]
                        ...
                        for dim in range(len(dataShape) - 1):
                            ...
                            dims.append(dimStr)
                        new_xrData = xr.DataArray(
                            data, coords=coords, dims=dims, name=name
                        )
                else:
                    print(
                        "was passed data which is neither xArray nor np. array. will not save ",
                        key,
                    )

                xrname = "%s/xr_%s_%s_Run%03d.nc" % (
                    self.dirname,
                    key.replace("/", "__"),
                    self.expname,
                    self.run,
                ...
                print(
                    "data for %s is only in memory, write to file with name: %s "
                    % (key, xrname)
                )
        ...
        for dirpath, dirnames, filenames in walk(self.dirname):
            for fname in filenames:
                if fname.find("xr") == 0 and fname.find("Run%03d" % self.run) >= 0:
                    try:
                        add_xrDataSet = xr.open_dataset(
                            self.dirname + "/" + fname, engine="h5netcdf"
                        )
                        key = fname.replace("xr_", "").replace(
                            "%s_Run%03d.nc" % (self.expname, self.run), ""
                        )
                        if key[-1] == "_":
                            key = key[:-1]
                        if len(add_xrDataSet[key].shape) > 2:
                            continue
                        if (
                            len(add_xrDataSet[key].shape) == 2
                            and add_xrDataSet[key].shape[1] < 10
                        ):
                            continue
                        key = key.replace("__", "/")
                        ...
                        print(
                            "found filename %s, added data for key %s " % (fname, key),
                            add_xrDataSet[key],
                        )
                        add_xrDataSet.close()
    ...
    def getOffVar(self, varName, selName, nNbr=3, mean=True, returnMe=False):
        ...
        if mean:
            self.addVar(
                "offNbrsAv_%s_%s_nNbr%02d" % (varName.replace("/", "_"), selName, nNbr),
                varArrayOff,
            )
        else:
            self.addVar(
                "offNbrs_%s_%s_nNbr%02d" % (varName.replace("/", "_"), selName, nNbr),
                varArrayOff,
            )
        if returnMe:
            return varArrayOff
    ...
    def printRunInfo(self):
        ...
        if isScan:
            ...
            if len(scanVar) == 1:
                print(
                    "this run is a scan of %s with %d points" % (scanVar[0], nPoints[0])
                )
            else:
        ...
        if varmin != varmax:
            ...
            if isinstance(varName, str):
                if varName in self.Sels:
                    ...
                    print(
                        "Cannot add cuts for filter %s as its not defined yet" % varName
                    )
    ...
    def removeCut(self, varName, useFilter):
        ...
        if not useFilter in self.Sels:
            print(
                "Selection with name %s does not exist, cannot remove cut" % useFilter
            )
            return
    ...
    def getFilterLaser(self, useFilter, ignoreVar=[]):
        ...
        return [
            self.getFilter(
                useFilter=useFilterBase + "__on", ignoreVar=ignoreVar
            ).squeeze(),
            self.getFilter(
                useFilter=useFilterBase + "__off", ignoreVar=ignoreVar
            ).squeeze(),
        ]
    ...
    def getFilter(self, useFilter=None, ignoreVar=[], debug=False):
        ...
        if useFilter == None or useFilterBase not in self.Sels.keys():
            if debug and useFilterBase not in self.Sels.keys():
                print(
                    "Selection %s is not available, defined Selections are:" % useFilter
                )
        ...
        if self.Sels[useFilterBase]._filter is not None and len(ignoreVar) == 0:
            ...
            if LaserReq == 1:
                return (
                    self.Sels[useFilterBase]._filter
                    & [self.getVar("lightStatus/laser") == 1]
                ).squeeze()
        ...
        for thiscut in self.Sels[useFilterBase].cuts:
            if not thiscut[0] in ignoreVar:
                ...
                if len(thiscut) == 3:
                    filters.append(
                        (thisPlotvar > thiscut[1]) & (thisPlotvar < thiscut[2])
                    )
                else:
                ...
                if debug:
                    ...
                    print(
                        f"getFilter: Cut {thiscut[1]} < {thiscut[0]} < {thiscut[2]} passes"
    ...
    def saveFilter(self, baseName="boolArray", useFilter=None, ignoreVar=[]):
        ...
        np.savetxt(
            "%s_Run%03d.txt" % (baseName, self.run),
            total_filter.astype(bool),
            fmt="%5i",
        )
        ...
        try:
            if Filter is None:
                if len(plotvar.split("/")) > 1:
                    ...
                    vals = self.fh5.get_node(
                        "/" + "/".join(plotvar.split("/")[:-1]), plotvar.split("/")[-1]
                    ).read()
                else:
    ...
    def setDelay(self, use_ttCorr=True, addEnc=False, addLxt=True, reset=False):
        delay = self.getDelay(
            use_ttCorr=use_ttCorr, addEnc=addEnc, addLxt=addLxt, reset=reset
        )
        ...
        if not isDaqDelayScan:
            if self.hasKey("enc/lasDelay"):
                ...
                if np.nanstd(encVal) > 1e-15 and np.nanstd(encVal) < 1e-9:
                    ...
                    print(
                        "strange encoder value for runs taken before encoder FEX....",
                        encCal.std(),
                    )
            else:
    ...
    def getPeak(
        ...
        if len(hst) == 2:
            ...
            print(
                "getPeak is not yet implemented for this type of data (need 1d histo)"
            )
        return
    ...
    def plotVar(
        ...
        if not isinstance(numBins, (list, tuple)):
            numBins = [numBins]
        if isinstance(plotvar, str) or (
            len(plotvar) == 2
            and (isinstance(plotvar[0], str) and not isinstance(plotvar[1], str))
        ):
            if len(numBins) != 1:
                print("bin# needs to be of same dimensions as plotvariables (1d)")
                return
            return self.plotVar1d(
                plotvar,
                numBins=numBins[0],
                useFilter=useFilter,
                limits=limits,
    ...
    def plotVar2d(
        ...
        for plotvar in plotvars:
            ...
            if len(vals[-1].shape) != 1:
                print(
                    "variable %s does not return a scalar, this is not yet implemented"
                    % plotvar
                )
        pmin0 = np.nanmin(vals[0])
        pmin1 = np.nanmin(vals[1])
        pmax0 = np.nanmax(vals[0])
        pmax1 = np.nanmax(vals[1])
        if limits[0] > 0:
        ...
        print(
            "plots %s from %g to %g and  %s from %g to %g "
            % (plotvars[0], pmin0, pmax0, plotvars[1], pmin1, pmax1)
        )
        ...
        ind2d = np.ravel_multi_index(
            (ind0, ind1), (binEdges0.shape[0] + 1, binEdges1.shape[0] + 1)
        )
        iSig = np.bincount(
            ind2d, minlength=(binEdges0.shape[0] + 1) * (binEdges1.shape[0] + 1)
    ...
    def getScanName(self):
        ...
        for key in self.Keys("scan"):
            if (
                key.find("var") < 0
                and key.find("none") < 0
                and key.find("damage") < 0
                and key.find("UserDataCfg") < 0
                and key.find("epics") < 0
            ):
        ...
        for scanVar in scanNames:
            if len(np.unique(self.getVar("scan/%s" % scanVar))) == 1:
                continue
            scanVarVary.append(scanVar)
        scanNames = scanVarVary
        if len(scanNames) == 0:
            return ""
        elif len(scanNames) == 1:
            return scanNames[0]
        else:
            return scanNames
        ...
        for scanVN in scanVarName:
            if scanVN.find("lxt") >= 0 or scanVN == "":
                ...
                try:
                    ...
                    scanValues.append([])
        if len(scanVarName) == 0:
            return "", []
        elif len(scanVarName) == 1:
            return scanVarName[0], scanValues[0]
        else:
            return scanVarName, scanValues
    ...
    def getBins(self, bindef=[], debug=False):
        ...
        if len(bindef) == 1:
            ...
            if scanVarName != "":
                ...
                if type(bindef[0]) is int:
                    Bins = np.linspace(
                        valBound[0], valBound[1], bindef[0], endpoint=True
                    )
                else:
        ...
        minEnc = int(min(scan) * 10.0)
        if minEnc < 0:
            minEnc += -1
        minEnc /= 10.0
        maxEnc = (int(max(scan) * 10.0) + 1) / 10.0
        ...
        if minEnc != maxEnc and abs(minEnc) < 101 and abs(maxEnc < 1001):
            ...
            print(
                "you passed only one number and the this does not look like a new delay scan or a normal scan"
            )
            return []

    def getScan(self, sig="", i0="", Bins=100, useFilter=None):
        return self.plotScan(
            sig=sig,
            i0=i0,
            Bins=Bins,
            returnData=True,
    ...
    def plotScan(
        ...
        if sig != "":
            ...
            print(
                "could not get signal variable %s, please specify"
                % plotVar.replace("__", "/")
            )
            return
        ...
        print(
            "from %i events keep %i (%i off events)"
            % (
                np.ones_like(i0Val).sum(),
                np.ones_like(i0Val[FilterOn]).sum(),
                np.ones_like(i0Val[FilterOff]).sum(),
            )
        )
        ...
        if (not self._delay_ttCorr) and (not self._delay_addEnc):
            ...
            scanPoints = np.concatenate(
                [scanPoints, [scanPoints[-1] + (scanPoints[1] - scanPoints[0])]], 0
            )
            usedDigitize = 1
        ...
        if binVar is not None:
            ...
            indOn2d = np.ravel_multi_index(
                (scanOnIdx, binIdx), (scanPoints.shape[0] + 1, binPoints.shape[0] + 1)
            )
            ...
            iNorm = (
                np.bincount(
                    indOn2d,
                    weights=i0Val[FilterOn],
                    minlength=(scanPoints.shape[0] + 1) * (binPoints.shape[0] + 1),
                )
                .reshape(scanPoints.shape[0] + 1, binPoints.shape[0] + 1)
                .T
            )
            iSig = (
                np.bincount(
                    indOn2d,
                    weights=sigVal[FilterOn],
                    minlength=(scanPoints.shape[0] + 1) * (binPoints.shape[0] + 1),
                )
                .reshape(scanPoints.shape[0] + 1, binPoints.shape[0] + 1)
                .T
            ...
            iNorm = np.bincount(
                scanOnIdx, i0Val[FilterOn], minlength=len(scanPoints) + 1
            )
            iSig = np.bincount(
                scanOnIdx, sigVal[FilterOn], minlength=len(scanPoints) + 1
            )
        ...
        if OffData:
            # same for off shots
            iNormoff = np.bincount(
                scanOffIdx, i0Val[FilterOff], minlength=len(scanOffPoints) + 1
            )
            iSigoff = np.bincount(
                scanOffIdx, sigVal[FilterOff], minlength=len(scanOffPoints) + 1
            )
        ...
        if OffData:
            if saveData:
                np.savetxt(
                    "Scan_Run%i.txt" % self.run,
                    (scanPoints, scan, scanOffPoints, scanoff),
                )
            returnDict = {
                "scanVarName": scanVarName,
                "scanPoints": scanPoints,
                "scan": scan,
                "scanOffPoints": scanOffPoints,
                "scanOff": scanoff,
                "plotVarName": plotVar,
            }
        else:
    ...
    def plotScanDict(
        ...
        if interpolation != "" and "scanOffPoints" in returnDict:
            finter_off = interpolate.interp1d(
                returnDict["scanOffPoints"], returnDict["scanOff"], kind=interpolation
            )
        ...
        if plotWith == "matplotlib":
            if len(scan.shape) > 1:
                extent = [
                    scanPoints.min(),
                    scanPoints.max(),
                    returnDict["binPoints"].min(),
                    returnDict["binPoints"].max(),
                ]
                plotImage(
                    scan,
                    ylim=[np.nanpercentile(scan, 1), np.nanpercentile(scan, 98)],
                    xLabel=scanVarName,
                    yLabel=returnDict["binVar"],
                    plotWith=plotWith,
                ...
                if (
                    plotDiff
                    and "scanOffPoints" in returnDict
                    and (interpolation != "" or len(scan) == len(returnDict["scanOff"]))
                ):
                    if fig is None:
                    ...
                    plotMarker(
                        ...
                        plotWith=plotWith,
                        runLabel=self.runLabel,
                        plotTitle="%s vs %s for %s"
                        % (plotVarName, scanVarName, self.runLabel),
                        plotDirname=self.plot_dirname,
                        markersize=5,
                    ...
                    plotMarker(
                        ...
                        markersize=5,
                        plotFilename=(
                            "Scan_diff_%s_vs_%s" % (plotVarName, scanVarName)
                        ),
                        line_dash="dashed",
                        color=["blue"],
                        fig=gs[1],
            ...
            if len(scan.shape) > 1:
                ...
                output_file = "%s/%s_Scan_%s_%s.html" % (
                    self.plot_dirname,
                    self.runLabel,
                ...
                if (
                    ...
                    layout = column(
                        Div(
                            text="<h1>%s as a function of %s for %s</h1>"
                            % (plotVarName, scanVarName, self.runLabel)
                        ),
                        grid,
                    )
            ...
            print(
                "plotting using %s is not implemented yet, options are matplotlib, bokeh_notebook, bokeh_html or no_plot"
            )
    ...
    def addCube(self, cubeName, binVar="", bins=[], useFilter="", addBinVars=None):
        ...
        self.cubes[cubeName] = Cube(
            binVar, bins, cubeName=cubeName, useFilter=useFilter, addBinVars=addBinVars
        )
    ...
    def addToCube(self, cubeName, targetVariable, isIdxVar=False):
        if cubeName in self.cubes.keys():
            ...
            print(
                "could not add variable %s to cube %s as this cube was not found"
                % (targetVariable, cubeName)
            )
    ...
    def getCube(self, cubeName):
        if cubeName in self.cubes.keys():
            ...
            print(
                "Cube %s not defined yet, options are: " % (cubeName), self.cubes.keys()
            )
            return None
    ...
    def prepCubeData(self, cubeName):
        ...
        if not (self.hasKey(cube.binVar) or cube.binVar == "delay"):
            print(
                "selection variable not in littleData, cannot make the data for this cube"
            )
            return None
        ...
        if len(cube.bins) > 3:
            ...
            if scanValues[0] != "" and len(scanValues[1]) > 0:
                ...
                print(
                    "prepCube could not prepare Bins: ",
                    scanValues[0] != "",
                    len(scanValues[1]),
                )
        else:
                ...
                if tVar.split(":")[0] == "droplet":  # ex: droplet:epix_2/droplet:image
                    ...
                    if dropBaseName not in cube.dropletProc.keys():
                        if (dropBaseName + "_x") in self._fields.keys():
                            ...
                            cube.dropletProc[dropBaseName]["vars"] = [
                                dropBaseName + "_x",
                                dropBaseName + "_y",
                                dropBaseName + "_adu",
                                dropBaseName + "_npix",
                            ...
                            # get shape for image.
                            cube.dropletProc[dropBaseName]["shape"] = (
                                np.nanmax(self.getVar("UserDataCfg/%s/ix" % dropDet)),
                                np.nanmax(self.getVar("UserDataCfg/%s/iy" % dropDet)),
                            )
                        else:
                            print(
                                "could not find droplets with base name: ", dropBaseName
                            )
                    else:
        ...
        if cube.binVar not in targetVarsLocal:
            targetVarsLocal.append(cube.binVar)
        ...
        if len(cube.useFilter.split("__")) > 1:
            if cube.useFilter.split("__")[1] == "on":
                onoff = 1
            elif cube.useFilter.split("__")[1] == "off":
                onoff = 0
        if (
            cube.useFilter.find(cube.cubeName) != 0
            and "%s_%s" % (cube.cubeName, useFilterBase) not in self.Sels.keys()
        ):
            self.Sels["%s_%s" % (cube.cubeName, useFilterBase)] = Selection()
            self.Sels["%s_%s" % (cube.cubeName, useFilterBase)].add(
                self.Sels[useFilterBase]
            )
            # if cube.binVar.find('lxt')>=0:
            #    Bins/=1e12
            self.Sels["%s_%s" % (cube.cubeName, useFilterBase)].addCut(
                cube.binVar, min(Bins), max(Bins)
            )
            for addVar in cube.addBinVars.keys():
                self.Sels["%s_%s" % (cube.cubeName, useFilterBase)].addCut(
                    addVar, min(cube.addBinVars[addVar]), max(cube.addBinVars[addVar])
                )
            ...
            for txVar in targetVarsLocal:
                ...
                if "damage/%s" % txVar in self._fields.keys():
                    self.Sels["%s_%s" % (cube.cubeName, cube.useFilter)].addCut(
                        "damage/%s" % txVar.split("/")[0], 0.5, 1.5
                    )
    ...
    def makeCubeData(
        ...
        if len(cube.addBinVars.keys()) > 0:
            ...
            if np.array(binIdx).min() < 1:
                printR(
                    rank,
                    "something went wrong in the setting of the cube selection, please fix me....",
                )
                return
        ...
        newXr = xr.merge(
            [
                newXr,
                xr.DataArray(
                    binVar, coords={"time": timeFiltered}, dims=("time"), name="binVar"
                ),
            ]
        )
        ...
        for tVar in cube.targetVars:
            ...
            if len(filteredVar.shape) == 1:
                newDar = xr.DataArray(
                    filteredVar, coords={"time": timeFiltered}, dims=("time"), name=tVar
                )
            else:
                coords = {"time": timeFiltered}
                dims = ["time"]
        ...
        if debug:
            print("make cube.binBounds later.", Bins)
        # now we actually bin.
        cubeData = newXr.groupby_bins(
            "binVar", Bins, labels=Bins[:-1], include_lowest=True, right=False
        ).sum(dim="time")

        # could add error using the std of the values.
        cubeDataErr = newXr.groupby_bins(
            "binVar", Bins, labels=Bins[:-1], include_lowest=True, right=False
        ).std(dim="time")
        ...
        if len(cube.addBinVars.keys()) > 0:
            ...
            for key in cubeData.variables:
                ...
                for addVar in cube.addBinVars.keys():
                    ...
                    dims.append(dim)
                    coords[dim] = 0.5 * (
                        cube.addBinVars[addVar][1:] + cube.addBinVars[addVar][:-1]
                    )
        ...
        if (
            not returnIdx
            and len(cube.addIdxVars) == 0
            and len(cube.dropletProc.keys()) == 0
        ):
            if toHdf5 == "h5netcdf":
                fname = "%s/Cube_%s_Run%03d_%s.nc" % (
                    self.dirname,
                    self.expname,
                    self.run,
                    cubeName,
        ...
        evtIDXr = xr.DataArray(
            self.getVar(fidVar, cubeFilter),
            coords={"time": timeFiltered},
            dims=("time"),
            name="fiducial",
        )
        evtIDXr = xr.merge(
            [
                evtIDXr,
                xr.DataArray(
                    self.getVar(evttVar, cubeFilter),
                    coords={"time": timeFiltered},
                    dims=("time"),
                    name="evttime",
                ),
        ...
        cubeIdxData = evtIDXr.groupby_bins(
            "binVar",
            Bins,
            ...
            if "image" in cube.dropletProc[droplet]["fct"]:
                ...
                for key in (Bins[1:] + Bins[:-1]) * 0.5:
                    if key in cubeIdxData.groups.keys():
                        ...
                        npix = evtIDXr[dropVar[3]][
                            cubeIdxData.groups[key]
                        ].data.flatten()
                        tx = evtIDXr[dropVar[0]][
                            cubeIdxData.groups[key]
                        ].data.flatten()[npix > 0]
                        ty = evtIDXr[dropVar[1]][
                            cubeIdxData.groups[key]
                        ].data.flatten()[npix > 0]
                        tadu = evtIDXr[dropVar[2]][
                            cubeIdxData.groups[key]
                        ].data.flatten()[npix > 0]
                        # make image
                        data = tadu
                        ...
                        col = ty
                        if (
                            max(row) >= cube.dropletProc[droplet]["shape"][0]
                            or max(col) >= cube.dropletProc[droplet]["shape"][1]
                        ):
                            if rank == 0:
                                print(
                                    "inconsistent shape ",
                        ...
                        imageAsMatrix = sparse.coo_matrix(
                            (data, (row, col)), shape=cube.dropletProc[droplet]["shape"]
                        ).todense()
                ...
                if len(cube.addBinVars.keys()) > 0:
                    ...
                    dataArray = xr.DataArray(
                        data, coords=coords, dims=dims, name=newKey
                    )
            ...
            if "array" in cube.dropletProc[droplet]["fct"]:
                ...
                for key in (Bins[1:] + Bins[:-1]) * 0.5:
                    if key in cubeIdxData.groups.keys():
                        ...
                        xArrayList.append(
                            evtIDXr[dropVar[0]][cubeIdxData.groups[key]].data.flatten()[
                                npix > 0
                            ]
                        )
                        yArrayList.append(
                            evtIDXr[dropVar[1]][cubeIdxData.groups[key]].data.flatten()[
                                npix > 0
                            ]
                        )
                        aduArrayList.append(
                            evtIDXr[dropVar[2]][cubeIdxData.groups[key]].data.flatten()[
                                npix > 0
                            ]
                        )
                ...
                for ia, (thisx, thisy, thisadu) in enumerate(
                    zip(xArrayList, yArrayList, aduArrayList)
                ):
                ...
                if len(cube.addBinVars.keys()) > 0:
                    ...
                    dataArray = xr.DataArray(
                        ...
                        coords=coords,
                        dims=dims,
                        name=((droplet + "_array_x").replace("/", "_")),
                    )
                    cubeData = xr.merge([cubeData, dataArray])
                    dataArray = xr.DataArray(
                        ydata,
                        coords=coords,
                        dims=dims,
                        name=((droplet + "_array_y").replace("/", "_")),
                    )
                    cubeData = xr.merge([cubeData, dataArray])
                    dataArray = xr.DataArray(
                        adudata,
                        coords=coords,
                        dims=dims,
                        name=((droplet + "_array_adu").replace("/", "_")),
                    )
                    cubeData = xr.merge([cubeData, dataArray])
    ...
    def AvImage(
        ...
        if useFilter is not None:
            ...
            dataAr = self.getVar(detname)[
                nSkip : min(nSkip + numEvts, self._tStamp.shape[0])
            ].squeeze()
    ...
    def plotAvImage(self, detname=None, ROI=[], limits=[5, 99.5]):
        ...
        print(
            "using the percentiles %g/%g as plot min/max: (%g, %g)"
            % (limits[0], limits[1], plotMin, plotMax)
        )
        ...
        if ROI != []:
            ...
            plt.subplot(gs[1]).imshow(
                img[ROI[0][0], ROI[1][0] : ROI[1][1], ROI[2][0] : ROI[2][1]],
    ...
    def FitCircle(self, detname=None, mask=None, method=None, thres=None):
        try:
            ...
            return
        print(
            "nearly done, but there is an issue in the image display and x/y"
        ...
        if len(image.shape) != 2:
            print(
                "not a 2-d image! Will return. image %s has %d dims"
                % (avImage, len(image.shape))
            )
            return
        ...
        if method not in ["m", "t"]:
            method = raw_input(
                "Please select m or p (mouse/threshold) or we will return\n"
            )
            if method not in ["m", "t"]:
                return
        ...
        if method == "m":
            ...
            while not happy:
                ...
                # draw the circle.
                circle = plt.Circle(
                    (res["xCen"], res["yCen"]), res["R"], color="b", fill=False
                )
            ...
            res = fitCircle(
                x.flatten()[imageThres.flatten().astype(bool)],
                y.flatten()[imageThres.flatten().astype(bool)],
            )
            print("res", res)
            print(
                "x,y av: ",
                (x.flatten()[imageThres.flatten().astype(bool)]).mean(),
    ...
    def azimuthalBinning(self, detname=None):
        print(
            " not yet implemented, exists in SmallDataProduced.py, uses code in xppmodules/src. Not sure if good idea"
        )
    ...
    def DropletCube(
        ...
        if len(dkey) == 0:
            print(
                "did not find any droplets in this smallData file: ", self.fh5.filename
            )
            return
        if len(dkey) > 1:
        ...
        ind3d = np.ravel_multi_index(
            (indA, indX, indY),
            (binAdu.shape[0] + 1, binX.shape[0] + 1, binY.shape[0] + 1),
        )
        cube = np.bincount(
            ind3d,
            minlength=(binAdu.shape[0] + 1) * (binX.shape[0] + 1) * (binY.shape[0] + 1),
        ).reshape(binAdu.shape[0] + 1, binX.shape[0] + 1, binY.shape[0] + 1)
    ...
    def makeDataArray(self, varList=[], useFilter=None):
        ...
        for tVar in varList:
            ...
            if len(filteredVar.shape) == 1:
                ...
                if newXr is None:
                    newXr = xr.DataArray(
                        filteredVar, coords=coords, dims=dims, name=tVar
                    )
                else:
                    newXr = xr.merge(
                        [
    ...
    def addEpicsArchiveVar(self, PVname, name=None, returnRaw=False):
        ...
        # now get the data.
        timePoints, valPoints = self._epicsArchive.get_points(
            PV=PVname, start=tStart_sec, end=tStop_sec, two_lists=True, raw=True
        )

        # add to data
        if name is None:
            name = PVname.replace(":", "_")

        # make a dataset with values * time points
        tStamp_epics = np.array([np.datetime64(int(tsec), "s") for tsec in timePoints])
        da_epics = xr.DataArray(
            valPoints, coords={"time": tStamp_epics}, dims=("time"), name=name
        )
        ...
        for epicsTime, epicsValue in zip(np.array(da_epics.time), da_epics.data):
            try:
                ...
                if epicsTime.astype(float) < binnedData.groups.keys()[0]:
                    nevents = len(
                        self.xrData.fiducials.time.data[
                            self.xrData.fiducials.time.data < epicsTime
                        ]
                    )
        ...
        da_epics_new = xr.DataArray(
            newArray,
            coords={"time": self.xrData.fiducials.time},
            dims=("time"),
            name=name,
        )
