...
def append_to_dataset(h5_dataset, data, is_sum_dataset=False):
    ...
    if is_sum_dataset:
        ...
        try:
            current_value = h5_dataset[()]  # Fixed variable name from dataset to h5_dataset
            h5_dataset[()] = current_value + data_array
        except ValueError as e:
            # Handle shape mismatch
            raise ValueError(f"Cannot sum data with shape {data_array.shape} to dataset "
        ...
        if h5_dataset.maxshape[0] is None or h5_dataset.maxshape[0] > h5_dataset.shape[0]:
            ...
            raise RuntimeError(f"Dataset {h5_dataset.name} is not resizable along the first dimension")
...
def create_dataset_from_key_value(
    ...
    if key in sum_dataset:  # New data will be summed
        ...
        dataset = parent.create_dataset(key, data=data, maxshape=(None,) + data.shape[1:])
        ...
        if False:
            ...
            if len(sample_shape) > 0:
                ...
                if len_data is not None:
                    ...
                    initial_shape = (0,) + sample_shape[1:]  # Start with 0 in first dimension
