...
class HsdObject(WaveformObject_lcls2): 
    ...
    def getData(self, evt):
        ...
        if self.wfx is None:
            self.wfx = [ datadict[k]['times'] for k in self.channels ]
            try:
                self.wfx = np.array(self.wfx)
            except:
                print('HsdObject: could not cast waveform times to array ',self.wfx)
        if self.evt.dat is None:
            self.evt.dat = [ datadict[k][0] for k in self.channels ]
            try:
