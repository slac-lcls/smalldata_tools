...
from smalldata_tools.lcls1.DetObject import DetObjectClass
...
class BaseSmallDataAna_psana(object):
    ...
    def __init__(
        ...
        if self.hutch == "cxi":
            print(
                "Will assume the first CXI station, if this is wrong, make sure to add  -e <expname> on commandline"
            )
        resp = requests.get(
            ws_url + "/lgbk/ws/activeexperiment_for_instrument_station",
            {"instrument_name": self.hutch.upper(), "station": 0},
        )
        try:
            currExpname = resp.json().get("value", {}).get("name")
            print("Current experiment for %s is %s" % (self.hutch, currExpname))
            rundoc = requests.get(
                ws_url + "/lgbk/" + expname + "/ws/current_run"
            ).json()["value"]
            if not rundoc:
            ...
            if self.run > lastRun and self.expname.find("tut") < 0:
                printR(
                    rank,
                    "experiment %s does only have %d runs, requested %d"
                    % (expname, lastRun, self.run),
                )
                return None
        except:
            lastRun = -1
        ...
        printR(
            rank,
            "\nTry to make SmallDataAna using dirname %s, for exp %s and run %s."
            % (dirname, expname, run),
        )
        try:
            printR(rank, "Setting up SmallData ana from anaps.")
            self.sda = sda.SmallDataAna(
                expname, self.run, dirname=dirname, filename=filename, plotWith=plotWith
            )
        except:
    ...
    def plotScan(
        self, ttCorr=False, sig="diodeU/channels", sigROI=[], i0="ipm3/sum", numBins=100
    ):
    ...
    def plotAvImage(
        ...
        print(
            f"plot {avImage} using the {limits[0]} {limits[1]} percentiles as plot min/max: ({plotMin}, {plotMax})"
        )
        ...
        if plotWith == "bokeh_notebook":
            ...
            im0 = plt.subplot(gs[0]).imshow(
                image, clim=[plotMin, plotMax], interpolation="None"
            )
    ...
    def saveAvImage(self, detname=None, dirname=""):
        ...
        for rawImg in avImages:
            ...
            if needsGeo:
                data_to_save[rawImg + "_image"] = self.__dict__[detname].det.image(
                    self.run, self.__dict__[rawImg]
                )
        if dirname == "":
    ...
    def SelectRegion(self, detname=None, limits=[5, 99.5]):
        ...
        plotMin = np.nanpercentile(img, limits[0])
        print(
            "plot %s using the %g/%g percentiles as plot min/max: (%g, %g)"
            % (avImage, limits[0], limits[1], plotMin, plotMax)
        )
        ...
        while not happy:
            ...
            plt.subplot(gs[1]).imshow(
                image[p_axis1[0] : p_axis1[1], p_axis0[0] : p_axis0[1]],
                clim=[plotMin, plotMax],
                interpolation="None",
            )
            plt.pause(0.0001)
        ...
        if not needsGeo:
            print(
                "ROI: [[%i,%i], [%i,%i]]"
                % (p[:, 1].min(), p[:, 1].max(), p[:, 0].min(), p[:, 0].max())
            )
            return
        ...
        if len(mask_nda.shape) > 2:
            for itile, tile in enumerate(mask_nda):
                if tile.sum() > 0:
                    ...
                    plt.imshow(
                        img[itile, ax1.min() : ax1.max(), ax0.min() : ax0.max()],
                        interpolation="none",
                    )
                    plt.pause(0.0001)
                    plt.show()
            ...
            if tile.sum() > 0:
                ...
                print(
                    "ROI: [[%i,%i], [%i,%i]]"
                    % (ax1.min(), ax1.max(), ax0.min(), ax0.max())
                )
        return

    def FitCircleAuto(
        self, detname=None, plotRes=True, forceMask=False, inImage={}, inParams={}
    ):
        if inImage != {}:
            img = inImage["image"]
            if "mask" in inImage:
            ...
            try:
                ...
                mask = (
                    self.__dict__[detname]
                    .det.mask(self.run, calib=True, status=True)
                    .astype(bool)
                )
            nPixRaw = 1.0
            ...
            print(
                ...
                mask.sum(),
                nPixRaw,
                (mask.sum() / nPixRaw) > 0.5,
                (mask.sum().astype(float) / nPixRaw),
            )
        ...
        needsGeo = False
        if (
            self.__dict__[detname].ped is not None
            and self.__dict__[detname].ped.shape != self.__dict__[detname].imgShape
        ):
            needsGeo = True
        ...
        plt.imshow(
            ...
            interpolation="none",
            cmap="gray",
            clim=[0, np.nanpercentile(img.flatten(), 99.5)],
        )
        ...
        for ir, thisRingInfo, rFit in zip(icount(), ringInfo, combRes["R"]):
            ...
            plt.plot(
                arSparse.col[thisRingInfo["pointsInCircle"]],
        ...
        # plt.gca().add_artist(circle1)
        plt.plot(
            [combRes["xCen"], combRes["xCen"]],
            [combRes["yCen"] - 20, combRes["yCen"] + 20],
            color="m",
        )
        plt.plot(
            [combRes["xCen"] - 20, combRes["xCen"] + 20],
            [combRes["yCen"], combRes["yCen"]],
            color="m",
        )
        plt.xlim(limits[0])
        plt.ylim(limits[1])
    ...
    def FitCircleMouse(
        self, detname=None, use_mask=False, limits=[5, 99.5], use_mask_local=False
    ):
        self.FitCircle(
            detname=detname,
            use_mouse=True,
    ...
    def FitCircle(self, **kwargs):
        ...
        if use_mouse is None or (
            not isinstance(use_mouse, bool) and not isinstance(use_mouse, basestring)
        ):
        ...
        if use_mask_local and self.__dict__.has_key("_mask_" + avImage):
            ...
            maskName = raw_input(
                "no local mask defined for %s, please enter file name " % avImage
            )

            if maskName != "" and os.path.isfile(maskName):
                locmask = np.loadtxt(maskName)
                if (
                    self.__dict__[detname].x is not None
                    and locmask.shape != self.__dict__[detname].x.shape
                ):
        ...
        if needsGeo:
            if self.__dict__[detname].det.dettype == DetObjectClass.Icarus:
                if singleTile < 0:
                    ...
                    print(
                        "requested tile %d, detector %s only has %d tiles"
                        % (singleTile, detname, image.shape[0])
                    )
                    return
        ...
        if use_mouse:
            while not happy:
                ...
                print(
                    "[x,y] (micron): [%f, %f] R (in mm): %f "
                    % (res["xCen"], res["yCen"], res["R"] / 1000.0)
                )
                if raw_input("Happy with this selection:\n") in ["y", "Y"]:
                    happy = True
            ...
            while not happy:
                if thresIn is not None:
                    ...
                    if not plotIt:
                        print(
                            "this is not going to work, you either need to specify a threshold or require plots"
                        )
                        return
                    fig = plt.figure(figsize=(10, 10))
                    if needsGeo:
            ...
            if singleTile < 0:
                res = fitCircle(
                    x.flatten()[img.flatten() > thresP],
                    y.flatten()[img.flatten() > thresP],
                )
            else:
                if len(x.shape) == len(img.shape) and len(x.shape) > 2:
                    res = fitCircle(
                        x[singleTile].flatten()[img[singleTile].flatten() > thresP],
                        y[singleTile].flatten()[img[singleTile].flatten() > thresP],
                    )
                else:
                    res = fitCircle(
                        x.flatten()[img[singleTile].flatten() > thresP],
    ...
    def MakeMask(self, detname=None, limits=[5, 99.5], singleTile=-1, extMask=None):
        ...
        while select:
            ...
            if shape == "r":
                ...
                if needsGeo:
                    ...
                    if self.__dict__[detname].det.dettype == DetObjectClass.Icarus:
                        maskTuple = [
                            mask_r_nda
                            for itile in range(self.__dict__[detname].ped.shape[0])
                        ]
                ...
                if raw_input("Select outer radius by mouse?\n") in ["y", "Y"]:
                    ...
                    r = rtot.split(" ")
                    ro = float(r[0])
                    ri = max(0.0, float(r[1]))
                mask_router_nda = np.array(
                    [(ix - cx) ** 2 + (iy - cy) ** 2 < ro**2 for ix, iy in zip(x, y)]
                )
                mask_rinner_nda = np.array(
                    [(ix - cx) ** 2 + (iy - cy) ** 2 < ri**2 for ix, iy in zip(x, y)]
                )
                ...
                nPoints = int(
                    raw_input("Number of Points (-1 until middle mouse click)?\n")
                )
                ...
                if shape == "d":
                    ...
                    if det.dettype in [
                        DetObjectClass.Jungfrau,
                        DetObjectClass.Epix10k,
                        DetObjectClass.Epix10k2M,
                        DetObjectClass.Epix10k2M_quad,
                    ]:
                        pedResult = pedResult[0]
                    hstPed = np.histogram(
                        pedResult.flatten(), np.arange(0, pedResult.max() * 1.05)
                    )
                else:
                    ...
                    if det.dettype in [
                        ...
                        pedResult = pedResult[0]
                    hstPed = np.histogram(
                        pedResult.flatten(), np.arange(0, pedResult.max() * 1.05, 0.05)
                    )
                if needsGeo:
                    pedResultImg = det.image(self.run, pedResult)
                else:
                    pedResultImg = pedResult.copy()
                plt.subplot(gsPed[0]).imshow(
                    pedResultImg,
                    clim=[
                        np.nanpercentile(pedResult, 1),
                        np.nanpercentile(pedResult, 99),
                    ],
                )
            ...
            if shape == "e":
                ...
                if needsGeo:
                    ...
                    plt.subplot(gs[1]).imshow(
                        det.image(self.run, mask_r_nda.astype(bool))
                    )
                else:
                    tile = mask_r_nda
                    tile[0:nEdge, :] = 1
            ...
            if shape == "cen" or shape == "center":
                ...
                if needsGeo:
                    for tile in mask_r_nda:
                        tile[
                            int(tile.shape[0] / 2 - nEdge) : int(
                                tile.shape[0] / 2 + nEdge
                            ),
                            :,
                        ] = 1
                        tile[
                            :,
                            int(tile.shape[1] / 2 - nEdge) : int(
                                tile.shape[1] / 2 + nEdge
                            ),
                        ] = 1
                    plt.subplot(gs[1]).imshow(
                        det.image(self.run, mask_r_nda.astype(bool))
                    ...
                    tile = mask_r_nda
                    tile[
                        int(tile.shape[0] / 2 - nEdge) : int(tile.shape[0] / 2 + nEdge),
                        :,
                    ] = 1
                    tile[
                        :,
                        int(tile.shape[1] / 2 - nEdge) : int(tile.shape[1] / 2 + nEdge),
                    ] = 1
                    plt.subplot(gs[1]).imshow(mask_r_nda.astype(bool))
            ...
            print(
                "masked in this step: ",
                np.ones_like(self.__dict__[detname].x)[mask_r_nda.astype(bool)].sum(),
            )
            print(
                "masked up to this step: ",
                np.ones_like(self.__dict__[detname].x)[totmask_nm1].sum(),
            )
            ...
            if len(mask) > 1:
                ...
                if needsGeo:
                    plt.subplot(gs2[1]).imshow(
                        det.image(self.run, image_mask), clim=[plotMin, plotMax]
                    )
                else:
        ...
        if extMask is not None:
            if extMask.shape != self.__dict__[detname].rawShape:
                print(
                    "extMask is the wrong shape: %s actual, %s desired."
                    % (extMask.shape, self.__dict__[detname].rawShape)
                )
                return None
            if extMask.dtype != np.dtype("int64"):
                print(
                    "extMask is the wrong type: %s actual, %s desired."
        ...
        if raw_input("Save to calibdir?\n") in ["y", "Y"]:
            srcStr = (
                det.source.__str__().replace('Source("DetInfo(', "").replace(')")', "")
            )
            ...
            det.save_txtnda(
                dirname + fname, mask.astype(float), fmt="%d", addmetad=True
            )
        elif raw_input("Save to local?\n") in ["y", "Y"]:
            if len(mask.shape) <= 2:
                np.savetxt(
                    "Mask_%s_%s_Run%03d.data" % (avImage, self.expname, int(self.run)),
                    mask,
                )
    ...
    def addAzInt(
        ...
        if center == None or len(center) != 2:
            centerString = raw_input(
                "please enter the coordinates of the beam center as c1,c2 or [c1,c2]:"
            )
            center = [
                float(centerString.replace("[", "").replace("]", "").split(",")[0]),
                float(centerString.replace("[", "").replace("]", "").split(",")[1]),
            ]

        azav = azimuthalBinning(
            center=center,
    ...
    def getAzAvs(self, detname=None):
        ...
        azintArray = [
            getattr(getattr(self, detname), key)
            for key in getattr(self, detname).__dict__.keys()
    ...
    def AzInt(
        ...
        if len(azIntegrations) > 1:
            ...
            azIntegs = [
                iiaz
                for iName, iiaz in zip(azIntNames, azIntegrations)
                if iName == azintName
            ]
            azInteg = azIntegs[0]
        else:
    ...
    def AzInt_centerVar(
        ...
        self.addAzInt(
            ...
            qBin=qBin,
            eBeam=9.5,
            center=[center[0] - varCenter, center[1]],
            dis_to_sam=dis_to_sam,
            name="cm10",
            inImage=inImage,
        )
        ...
        self.addAzInt(
            ...
            qBin=qBin,
            eBeam=9.5,
            center=[center[0], center[1] - varCenter],
            dis_to_sam=dis_to_sam,
            name="cm01",
            inImage=inImage,
        )
        ...
        self.AzInt(
            detname=detname, use_mask=use_mask, azintName="cm10", inImage=inImage
        )
        self.AzInt(
            detname=detname, use_mask=use_mask, azintName="cp10", inImage=inImage
        )
        self.AzInt(
            detname=detname, use_mask=use_mask, azintName="cm01", inImage=inImage
        )
        self.AzInt(
            detname=detname, use_mask=use_mask, azintName="cp01", inImage=inImage
        )
        ...
        while 1:
            ymin = 0
            ymax = azintValues_c00.shape[1] - 1
            ...
            if zoom is not None:
                if len(zoom) == 1:
                    ...
                    if (
                        isinstance(zoom, list)
                        and isinstance(zoom[0], int)
                        and zoom[0] >= 0
                        and zoom[1] >= 0
                        and zoom[0] != zoom[1]
                    ):
                        ...
                        yString = raw_input(
                            "please enter x-boundaries of the zoomed figure as c1,c2 or [c1,c2]:"
                        )
                        ymin = int(
                            yString.replace("[", "").replace("]", "").split(",")[0]
                        )
                        ymax = int(
                            yString.replace("[", "").replace("]", "").split(",")[1]
                        )
            print("we will plot from bin %d to %d " % (ymin, ymax))

            try:
                p33_11.imshow(
                    azintValues_c00[:, ymin:ymax],
                    aspect="auto",
                    interpolation="none",
                    vmax=maxVal,
                )
                p33_10.imshow(
                    azintValues_cm10[:, ymin:ymax],
                    aspect="auto",
                    interpolation="none",
                    vmax=maxVal,
                )
                p33_12.imshow(
                    azintValues_cp10[:, ymin:ymax],
                    aspect="auto",
                    interpolation="none",
                    vmax=maxVal,
                )
                p33_01.imshow(
                    azintValues_cm01[:, ymin:ymax],
                    aspect="auto",
                    interpolation="none",
                    vmax=maxVal,
                )
                p33_21.imshow(
                    azintValues_cp01[:, ymin:ymax],
                    aspect="auto",
                    interpolation="none",
                    vmax=maxVal,
                )
                maxVal_c00 = np.array(
                    [
                        [ip, np.nanargmax(phiBin)]
                        for ip, phiBin in enumerate(azintValues_c00[:, ymin:ymax])
                    ]
                )
                maxVal_cm10 = np.array(
                    [
                        [ip, np.nanargmax(phiBin)]
                        for ip, phiBin in enumerate(azintValues_cm10[:, ymin:ymax])
                    ]
                )
                maxVal_cm01 = np.array(
                    [
                        [ip, np.nanargmax(phiBin)]
                        for ip, phiBin in enumerate(azintValues_cm01[:, ymin:ymax])
                    ]
                )
                maxVal_cp10 = np.array(
                    [
                        [ip, np.nanargmax(phiBin)]
                        for ip, phiBin in enumerate(azintValues_cp10[:, ymin:ymax])
                    ]
                )
                maxVal_cp01 = np.array(
                    [
                        [ip, np.nanargmax(phiBin)]
                        for ip, phiBin in enumerate(azintValues_cp01[:, ymin:ymax])
                    ]
                )
                try:
                    ...
                    if plotGrid == 0:
                        cMaxVal_c00 = np.nanargmax(
                            azintValues_c00[:, ymin:ymax].sum(axis=0)
                        )
                        p33_11.plot(
                            [cMaxVal_c00, cMaxVal_c00],
                            [0, azintValues_c00.shape[0]],
                            "m",
                        )
                        p33_10.plot(
                            [cMaxVal_c00, cMaxVal_c00],
                        ...
                        for il, lineVal in enumerate(
                            np.linspace(ymin, ymax, int(plotGrid))
                        ):
                            lw = 1
    ...
    def SelectRegionDroplet(self, detname=None, limits=[5, 99.5]):
        ...
        if raw_input("Save to calibdir?\n") in ["y", "Y"]:
            ...
            if det.dettype == DetObjectClass.CsPad:
                dirname = "/reg/d/psdm/%s/%s/calib/CsPad2x2::CalibV1/%s/pixel_mask/" % (
                    self.sda.expname[:3],
                    self.sda.expname,
                    srcStr,
                )
            elif det.dettype == DetObjectClass.CsPad2M:
                dirname = "/reg/d/psdm/%s/%s/calib/CsPad::CalibV1/%s/pixel_mask/" % (
                    self.sda.expname[:3],
                    self.sda.expname,
                    srcStr,
                )
            elif det.dettype == DetObjectClass.Epix:
                dirname = "/reg/d/psdm/%s/%s/calib/Epix100a::CalibV1/%s/pixel_mask/" % (
                    self.sda.expname[:3],
                    self.sda.expname,
                    srcStr,
                )
            elif det.dettype == DetObjectClass.Epix10k:
                dirname = "/reg/d/psdm/%s/%s/calib/Epix10ka::CalibV1/%s/pixel_mask/" % (
                    self.sda.expname[:3],
                    self.sda.expname,
                    srcStr,
                )
                ...
                dirname = (
                    "/reg/d/psdm/%s/%s/calib/Epix10ka2M::CalibV1/%s/pixel_mask/"
                    % (self.sda.expname[:3], self.sda.expname, srcStr)
                )
            if not os.path.exists(dirname):
                os.makedirs(dirname)
            fname = "%s-end.data" % self.run
