...
class DetObjectClass(object):
    ...
    def params_as_dict(self):
        ...
        parList = {
            ...
            if (
                key[0] != "_"
                and isinstance(getattr(self, key), (basestring, int, float, np.ndarray))
            )
        }
        parList.update(
            {
                ...
                if (
                    ...
                    and isinstance(
                        getattr(self, key)[0], (basestring, int, float, np.ndarray)
                    )
                )
            }
        )
            ...
            parList.update(
                {
                    ...
                    if (
                        key[0] != "_"
                        and isinstance(
                            value, (basestring, int, float, np.ndarray, tuple)
                        )
                    )
                }
            )
            ...
            try:
                ...
                if len(self.ped.shape) > 2:
                    try:
                        ...
                        print(
                            "multi dim pedestal & image function does do nothing: multi gain detector....."
                        )
    ...
    def _get_coords_from_ped(self):
        self.x = np.arange(
            0, self.ped.shape[-2] * self.pixelsize[0] * 1e6, self.pixelsize[0] * 1e6
        )
    ...
    def _getMasks(self):
        try:
            ...
            if rank == 0 and self.mask is not None:
                print(
                    "masking %d pixel (status & edge,..) of %d"
                    % (
                        np.ones_like(self.mask).sum() - self.mask.sum(),
            ...
            if (
                ...
                and self.cmask.sum() != self.mask.sum()
                and rank == 0
            ):
                print(
                    "found user mask, masking %d pixel"
                    % (np.ones_like(self.mask).sum() - self.cmask.sum())
                )
        except:
            self.mask = None
            self.cmask = None
    ...
    def getMask(self, ROI):
        ...
        if self._needsGeo:
            mask_r_nda = np.array(
                [mask_roi[ix, iy] for ix, iy in zip(self.ix, self.iy)]
            )
        else:
            mask_r_nda = mask_roi
    ...
    def processFuncs(self):
        ...
        for func in [
            ...
            try:
                ...
                print(
                    "Could not run function %s on data of detector %s of shape"
                    % (func._name, self._name),
                    self.evt.dat.shape,
                )
...
class OpalObject(CameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        if (
            env.configStore().get(psana.TimeTool.ConfigV2, self._src) is not None
            and env.configStore().get(psana.TimeTool.ConfigV2, self._src).write_image()
            == 0
        ):
            ...
            if (
                fexCfg.forwarding() == fexCfg.Forwarding.values[2]
            ):  # make sure we are only doing ROI
                self.ped = np.zeros(
                    [
                        fexCfg.roiEnd().column() - fexCfg.roiBegin().column(),
                        fexCfg.roiEnd().row() - fexCfg.roiBegin().row(),
                    ]
                ).T
            if self.ped is None or self.ped.shape == (
                0,
                0,
            ):  # this is the case for e.g. the xtcav recorder but can also return with the DAQ. Assume Opal1k for now.
                # if srcName=='xtcav':
...
class ZylaObject(CameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        if self.imgShape != self.ped.shape:
            print(
                "Detector %s does not have a pedestal taken for run %s, this might lead to issues later on!"
                % (self._name, run)
            )
            if self.common_mode != -1:
                print(
                    "We will use the raw data for Detector %s in run %s"
                    % (self._name, run)
                )
            self.common_mode = -1
        self.imgShape = self.ped.shape
...
class ControlsCameraObject(CameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        camrecCfg = env.configStore().get(
            psana.Camera.ControlsCameraConfigV1, self._src
        )
...
class OrcaObject(CameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        try:
            ...
            self.ped = np.zeros(
                [
                    ofCfg.roiEnd().row() - ofCfg.roiBegin().row(),
...
class IcarusObject(CameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        self._common_mode_list = [
            ...
            99,
            -1,
        ]  # none, unb-calc, unb-applied, raw
        self.common_mode = kwargs.get("common_mode", self._common_mode_list[0])
        if self.common_mode is None:
            self.common_mode = self._common_mode_list[0]
        if self.common_mode not in self._common_mode_list:
            print(
                "Common mode %d is not an option for an Icarus detector, please choose from: "
                % self.common_mode,
                self._common_mode_list,
            )
        if self.x is None and self.ped is not None:
            self.x = (
                np.arange(0, self.ped.shape[-2] * self.pixelsize[0], self.pixelsize[0])
                * 1e6
            )
            self.y = (
                np.arange(0, self.ped.shape[-1] * self.pixelsize[0], self.pixelsize[0])
                * 1e6
            )
    ...
    def getData(self, evt):
        ...
        if (
            self.common_mode >= 80 and self.common_mode < 100
        ):  # this here is some icarus stuff.
            self.evt.dat = self.det.calib(evt)
            ...
            if self.cm_maskedROI is not None:
                cm_mask_mean = [
                    np.nanmean(
                        frame[
                            self.cm_maskedROI[0] : self.cm_maskedROI[1],
                            self.cm_maskedROI[2] : self.cm_maskedROI[3],
                        ]
                    )
                    for frame in self.evt.dat
                ]
                cm_mask_med = [
                    np.nanmedian(
                        frame[
                ...
                dataFrame = np.array(
                    [df - corr for df, corr in zip(self.evt.dat, cm_mask_med)]
                )
            ...
            if self.common_mode == 81:
                corrImg, cmDict = cm_uxi(
                    dataFrame,
                    self.cm_photonThres,
                    self.cm_maxCorr,
                    self.cm_minFrac,
                    self.cm_maskNeighbors,
                )
...
class CsPadObject(TiledCameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        if self.common_mode not in self._common_mode_list:
            print(
                "Common mode %d is not an option for a CsPad detector, please choose from: "
                % self.common_mode,
                self._common_mode_list,
            )
        ...
        if self.gain_mask.std() != 0:
            ...
            self.gain_mask *= 6.85
            self.gain_mask[self.gain_mask == 0] = 1.0
    ...
    def getData(self, evt):
        ...
        if self.common_mode % 100 == 1:
            ...
            try:
                cmValues = self.det.common_mode_correction(
                    self.run,
                    self.det.calib(evt, cmpars=(0), mbits=mbits),
                    cmpars=(1, 25, 40, 100, 0),
                    mbits=mbits,
                )
                self.evt.__dict__["env_cmValues"] = np.array(
                    [tile[0][0] for tile in cmValues]
                )
            except:
                pass
...
class CsPad2MObject(CsPadObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        if self.common_mode not in self._common_mode_list:
            print(
                "Common mode %d is not an option for a CsPad2M detector, please choose from: "
                % self.common_mode,
                self._common_mode_list,
            )
        if self.ped is not None:
...
class EpixObject(TiledCameraObject):
    ...
    def getData(self, evt):
        ...
        if self.common_mode % 100 == 6:
            self.evt.dat = self.det.calib(
                evt, cmpars=[6], mbits=mbits, rms=self.rms, normAll=True
            )
            needGain = False
        ...
        try:
            envRow = evt.get(
                psana.Epix.ElementV3, psana.Source(self.det.alias)
            ).environmentalRows()[1]
...
class Epix10kObject(TiledCameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        self._common_mode_list = [
            ...
            -2,
            30,
        ]  # official, ped sub, raw, raw_gain, calib
        self.common_mode = kwargs.get("common_mode", self._common_mode_list[0])
        if self.common_mode is None:
            self.common_mode = self._common_mode_list[0]
        if self.common_mode not in self._common_mode_list:
        ...
        self.nomGain = [
            ...
            3.0,
            100.0,
        ]  # H,M,L,(HL auto), (ML auto - 2 values)
        # asicList=[0,3,1,2]
        asicList = [0, 1, 3, 2]
        self.carrierId0 = epixCfg.carrierId0()
        self.carrierId1 = epixCfg.carrierId1()
        ...
        cfgReshape = epixCfg.asicPixelConfigArray().reshape(
            int(cfgShape[0] / 2), cfgShape[1] * 2, order="F"
        )
        pixelGain = np.ones_like(cfgReshape).astype(float)
        for ia in asicList:
            if epixCfg.asics(ia).trbit() == 1:
                continue
            asicGainConfig = cfgReshape[
                :, ia * int(cfgShape[1] / 2) : (ia + 1) * int(cfgShape[1] / 2)
            ]
            pixelGain[
                :, ia * int(cfgShape[1] / 2) : (ia + 1) * int(cfgShape[1] / 2)
            ] = ((asicGainConfig & 0x4) / 4).astype(float) * self.nomGain[1] + (
                (np.ones_like(asicGainConfig) - (asicGainConfig & 0x4) / 4)
            ).astype(
                float
            ) * self.nomGain[
                2
    ...
    def getData(self, evt):
        ...
        if self.common_mode < 0:
            ...
            try:
                epixCalRows = evt.get(
                    psana.Epix.ArrayV1, psana.Source(self.det.alias)
                ).calibrationRows()
            ...
            self.evt.dat = np.empty(
                evt_dat.shape, dtype=np.float32
            )  # can't reember why I'm doing this. I think for the data type.
        ...
        try:
            ...
            for moduleRow in envRows:
                tempVal = np.array(
                    [divmod(temp, 1 << 16) for temp in moduleRow[0]]
                ).flatten()
                ...
                anatemp.append(
                    moduleRow[1][33] * 2.048 / 4095 * (130 / (0.882 - 1.951))
                    + (0.882 / 0.0082 + 100)
                )
                digtemp.append(
                    moduleRow[1][30] * 2.048 / 4095 * (130 / (0.882 - 1.951))
                    + (0.882 / 0.0082 + 100)
                )
...
class Epix10k2MObject(TiledCameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        self._common_mode_list = [
            ...
            -2,
            30,
        ]  # official, sn kludge, ped sub, raw, calib
        self.common_mode = kwargs.get("common_mode", self._common_mode_list[0])
        if self.common_mode is None:
            self.common_mode = self._common_mode_list[0]
        if self.common_mode not in self._common_mode_list:
        ...
        for i in range(epixCfg.elemCfg_shape()[0]):
            ...
            cfgReshape = elemCfg.asicPixelConfigArray().reshape(
                int(cfgShape[0] / 2), cfgShape[1] * 2, order="F"
            )
            pixelGain = np.ones_like(cfgReshape).astype(float)
            for ia in asicList:
        ...
        if self.common_mode > 100:
            ...
            calibDir = (
                "/reg/d/psdm/%s/%s/calib/Epix10ka2M::CalibV1/XcsEndstation.0:Epix10ka2M.0/ghost"
                % (env.experiment()[:3], env.experiment())
            )
            ghostFile = tables.open_file(
                "%s/GhostFits_%s_Run%03d.h5" % (calibDir, env.experiment(), run)
            )
            ...
            self.dsIdxRun = (
                psana.DataSource("exp=%s:run=%d:idx" % (env.experiment(), run))
                .runs()
                .next()
            )
            self.dsIdxSecs = []
            ...
            self.preEvtDet = DetObject(
                self.det.alias, env, run, common_mode=self.common_mode % 100
            )
    ...
    def getData(self, evt):
        ...
        if self.common_mode < 0:
            ...
            self.common_mode % 100 == 80
        ):  # placeholder for specific treatment of epix10k
            evt_dat = self.det.calib(evt, cmpars=(7, 0, 100))
            self.evt.dat = np.empty(
                evt_dat.shape, dtype=np.float32
            )  # can't reember why I'm doing this. I think for the data type.
            ...
            self.common_mode % 100 == 81
        ):  # my horrible kludge that might work for fixed gains.
            if len(self.ped.shape) > 3:
                ...
                if self.gainSetting == 0:
                    ...
                    self.evt.dat = (
                        dat_dat[gain_dat == 0] + dat_dat[gain_dat > 1] * self.nomGain[3]
                    )
                else:
                    self.evt.dat = (
                        dat_dat[gain_dat == 0] * self.nomGain[4]
                        + dat_dat[gain_dat > 1] * self.nomGain[5]
                    )
        ...
        if (
            self.common_mode > 100
        ):  # correct for the ghost image after official calibration
            # current event info
...
class RayonixObject(CameraObject):
    def __init__(self, det, env, run, **kwargs):
        ...
        rcfg = env.configStore().get(
            psana.Rayonix.ConfigV2, psana.Source("%s" % det.name)
        )
        ...
        try:
            ...
            self.pixelsize = [
                170e-3 / 3840 * binning
            ]  # needs to change for bigger rayonix.
        try:
        ...
        if self.x is None:
            self.x = (
                np.arange(0, self.ped.shape[0] * self.pixelsize[0], self.pixelsize[0])
                * 1e6
            )
            self.y = (
                np.arange(0, self.ped.shape[0] * self.pixelsize[0], self.pixelsize[0])
...
class UxiObject(DetObjectClass):
    ...
    def getData(self, evt):
        ...
        if len(np.argwhere(self._uxiDict["lcls_ts_secs"] == evttime[0])) > 0:
            ...
            if (
                len(np.argwhere(self._uxiDict["lcls_ts_necs"] == evttime[1])) > 0
                and len(np.argwhere(self._uxiDict["lcls_ts_fids"] == evtfid)) > 0
            ):
                # evtNr_withUxi=eventNr
                # evtFid_withUxi=evtfid
                if (
                    evttime[1] == self._uxiDict["lcls_ts_necs"][secIdx]
                    and evtfid == self._uxiDict["lcls_ts_fids"][secIdx]
                ):
                    uxiEvent = True
                    uxiIdx = secIdx
        ...
        try:
            ...
            print(
                "Uxi pedestal subtraction failed: shapes:",
                dataFrame.shape,
                self.ped.shape,
            )
        ...
        if self.cm_maskedROI is not None:
            ...
            cm_mask_mean = [
                np.nanmean(
                    frame[maskedROI[0] : maskedROI[1], maskedROI[2] : maskedROI[3]]
                )
                for frame, maskedROI in zip(dataFrame, self.cm_maskedROI)
            ]
            cm_mask_med = [
                np.nanmedian(
                    frame[maskedROI[0] : maskedROI[1], maskedROI[2] : maskedROI[3]]
            ...
            self.evt.__dict__["env_cm_masked_std"] = np.array(cm_mask_std)
            dataFrame = np.array(
                [df - corr for df, corr in zip(dataFrame, cm_mask_med)]
            )
