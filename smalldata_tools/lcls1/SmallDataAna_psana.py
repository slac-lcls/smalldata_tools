...
class SmallDataAna_psana(BaseSmallDataAna_psana):
    def __init__(
        ...
        if self.dsnameIdx is None:
            printR(
                rank,
                "Failed to set up index-based psana datasource, likely because this run is still live",
            )
        else:
            try:
                ...
                printR(
                    rank,
                    "Failed to set up index based psana dataset from dsname: %s"
                    % self.dsnameIdx,
                )
            if self.dsIdx is not None:
                try:
                    ...
                    try:
                        ...
                        printR(
                            rank,
                            "Failed to get run from psana dataset from dsname: %s"
                            % self.dsnameIdx,
                        )
        return
    ...
    def Keys(self, printthis=False):
        try:
            ...
            try:
                ...
                printR(
                    rank,
                    "WARNING: smd data also did not work, give up. No keys returned! ",
                )
    ...
    def addDetInfo(self, detname="None", common_mode=None):
        ...
        if (
            detname in self.__dict__.keys()
            and self.__dict__[detname].common_mode == common_mode
            and detnameDict is not None
        ):
            return detname
        ...
        if detname in self.__dict__.keys():
            printR(
                rank,
                "redefine detector object with different common mode:"
                "%d instead of %d" % (common_mode, self.__dict__[detname].common_mode),
            )
        det = DetObject(
            detname, self.dsIdx.env(), self.run, name=detname, common_mode=common_mode
        )
        ...
        if detnameDict is not None:
            for key in detnameDict.keys():
                ...
                if key.find("ROI") == 0:
                    self.__dict__[detname].addFunc(
                        ROIFunc(name=key, ROI=detnameDict[key], writeArea=True)
                    )
                if key.find("photon") == 0:
                    ...
                    self.__dict__[detname].addFunc(
                        photon(
                            ADU_per_photon=detnameDict[key], thresADU=thres, retImg=2
                        )
                    )
    ...
    def getImages(
        self, detname="None", numEvts=-1, useFilter=None, nSkip=0, common_mode=0
    ):
        if not isinstance(detname, basestring):
            print(
                "please give parameter name unless specifying arguments in right order. detname is first"
            )
            return
        # look for detector
        if detname == "None":
        ...
        if (
            not detname in self.__dict__.keys()
            or self.__dict__[detname].common_mode != common_mode
        ):
        ...
        if detname.find("opal") >= 0:
            ...
            if pedestals[0, 0] == -1:
                print(
                    "time tool opal, image was not saved. Should ideally exclude from detector list"
                )
                return
        else:
        ...
        if useFilter is not None:
            ...
            print(
                "using smd based selection, have %s events for selection %s"
                % (len(evttsSel), useFilter)
            )
            if numEvts == -1:
        ...
        print(
            ...
            numEvts,
            " used ",
            min(len(times), numEvts),
            " now actually get events",
        )
    ...
    def _getEventTimestamps(
        self, numEvts=100, useFilter=None, nSkip=0, uniform=False, printFid=False
    ):
        # now get the non-image data
        run = self.dsIdxRun
        times = []
        ...
        if useFilter is not None:
            ...
            times = run.times()[nSkip:]
        print(
            "requested %d, used %d now actually get events"
            % (numEvts, min(len(times), numEvts))
        )
        if min(len(times), numEvts) < numEvts * 0.5:
            if raw_input("too few events, quit?") in ["y", "Y", "yes", "Yes"]:
                return
    ...
    def pixelHistogram(
        ...
        imgAr = []
        run = self.dsIdxRun
        times = self._getEventTimestamps(
            numEvts=numEvts,
            useFilter=useFilter,
            nSkip=nSkip,
            uniform=uniform,
        ...
        if maxHis is not None:
            bmax = np.max(maxHis, bmax)
    ...
    def AvImage(
        ...
        for tm in times:
            ...
            if minIpm != -1 and (
                (
                    self.hutch == "xpp"
                    and evt.get(
                        psana.Lusi.IpmFexV1, psana.Source("BldInfo(XppSb2_Ipm)")
                    ).sum()
                    < minIpm
                )
                or (
                    self.hutch == "xcs"
                    and evt.get(
                        psana.Lusi.IpmFexV1, psana.Source("BldInfo(XCS-IPM-05)")
                    ).sum()
                    < minIpm
                )
            ):
            ...
            for dval in det.evt.__dict__:
                if dval.find("env") >= 0:
                    ...
                    if isinstance(det.evt.__dict__[dval], list) or isinstance(
                        det.evt.__dict__[dval], np.ndarray
                    ):
                        envDict[dval.replace("env_", "")].append(
                            det.evt.__dict__[dval][0]
                        )
                    else:
        ...
        if thresRms != 0:
            data += "thresRms%d_" % int(thresRms * 10.0)
    ...
    def makePedestal(
        ...
        if useFilter is not None:
            ...
            if minFrac > 0.05 and i0List != []:
                print(
                    "This selection seems to lets too many events with beam through, will quit"
                )
                return
        ...
        if useFilter is not None:
            pedImg = self.__dict__[
                "AvImg_median_Filter%s_raw_%s" % (useFilter, detname)
            ]
        ...
        det.save_txtnda(
            dirname + "pixel_status/" + fname, status, fmt="%d", addmetad=True
        )
    ...
    def plotCalib(self, detname="None", common_mode=0, plotWith=None):
        ...
        hstRms = []
        if (
            det.dettype == DetObjectClass.Jungfrau
            or det.dettype == DetObjectClass.Icarus
        ):
            for ped, frms in zip(pedestals, rms):
                hstPed.append(
                    np.histogram(ped.flatten(), np.arange(0, pedestals.max() * 1.05))
                )
                hstRms.append(
                    np.histogram(frms.flatten(), np.arange(0, rms.max() * 1.05, 0.05))
                )
        else:
            hstPed.append(
                np.histogram(pedestals.flatten(), np.arange(0, pedestals.max() * 1.05))
            )
            hstRms.append(
                np.histogram(rms.flatten(), np.arange(0, rms.max() * 1.05, 0.05))
            )
        ...
        if plotWith.find("bokeh") >= 0:
            ...
            hstPedO = hv.Overlay(
                [
                    hv.Scatter(
                        (0.5 * (hst[1][:-1] + hst[1][1:]), np.log(hst[0])),
                        "pedestal",
                        "log(nEntries)",
                    )
                    for hst in hstPed
            ...
            imgPed = hv.Image(
                pedImg.T, bounds=bounds, vdims=[hv.Dimension("pedData", range=zPed)]
            ).options(cmap="viridis", colorbar=True, invert_yaxis=True, width=plotwidth)
            imgRms = hv.Image(
                rmsImg.T, bounds=bounds, vdims=[hv.Dimension("rmsData", range=zRms)]
            ).options(cmap="viridis", colorbar=True, invert_yaxis=True, width=plotwidth)
        ...
        if det.dettype == DetObjectClass.Jungfrau:
            im0 = plt.subplot(gsPed[0]).imshow(
                pedImg,
                clim=[
                    np.nanpercentile(pedestals[0], 1),
                    np.nanpercentile(pedestals[0], 99),
                ],
            )
            cbar0 = plt.colorbar(im0)
            im2 = plt.subplot(gsPed[2]).imshow(
                rmsImg, clim=[np.nanpercentile(rms[0], 1), np.nanpercentile(rms[0], 99)]
            )
        else:
            im0 = plt.subplot(gsPed[0]).imshow(
                pedImg,
                clim=[np.nanpercentile(pedestals, 1), np.nanpercentile(pedestals, 99)],
            )
            cbar0 = plt.colorbar(im0)
            im2 = plt.subplot(gsPed[2]).imshow(
                rmsImg, clim=[np.nanpercentile(rms, 1), np.nanpercentile(rms, 99)]
            )
        cbar2 = plt.colorbar(im2)

    def _fillCalibHisto(
        self, detname="None", printVal=[-1], pedBinWidth=10, rmsBinWidth=0.1
    ):
        ...
        for ipedRun, pedRun in enumerate(pedRuns):
            ...
            if needsGeo:
                try:
                    if (
                        detTypeStr == "Jungfrau::CalibV1"
                        or detTypeStr == "Epix10ka2M::CalibV1"
                    ):
            ...
            if len(printVal) < 2:
                ...
                print(
                    ...
                    % (
                        pedRun,
                        (allPeds[-1] < printVal[0]).sum(),
                        (allPeds[-1] > printVal[1]).sum(),
                        (allRms[-1] < printVal[2]).sum(),
                        (allRms[-1] > printVal[3]).sum(),
                    )
                )
            elif len(printVal) >= 2:
                print(
                    "run %d, pixel cold/hot: %d / %d pixels"
                    % (
                        pedRun,
                        (allPeds[-1] < printVal[0]).sum(),
                        (allPeds[-1] > printVal[1]).sum(),
        ...
        for thisPed, thisRms in zip(allPeds, allRms):
            ped2d.append(
                np.histogram(
                    thisPed.flatten(),
                    np.arange(pedHisLow * 0.8, pedHisHigh * 1.2, pedBinWidth),
                )[0]
            )
            ...
            rms2d.append(
                np.histogram(
                    thisRms.flatten(),
                    np.arange(rmsHisLow * 0.8, rmsHisHigh * 1.2, rmsBinWidth),
                )[0]
            )
    ...
    def plotCalibHisto(
        ...
        if "rms3d" not in self.calibhisto.keys():
            self._fillCalibHisto(
                detname=detname,
                printVal=printVal,
                pedBinWidth=pedBinWidth,
                rmsBinWidth=rmsBinWidth,
            )
        if plotWith is None:
            plotWith = self.plotWith
        ...
        if plotWith == "bokeh_notebook":
            ...
            boundsPed = (
                0,
                np.array(self.calibhisto["runs"]).min(),
                allPeds[icurrRun].max(),
                np.array(self.calibhisto["runs"]).max(),
            )
            pedZ = (
                np.nanpercentile(self.calibhisto["ped2d"], 1),
                np.nanpercentile(self.calibhisto["ped2d"], 99.5),
            )
            boundsRms = (
                0,
                np.array(self.calibhisto["runs"]).min(),
            ...
            rmsZ = (
                np.nanpercentile(self.calibhisto["rms2d"], 1),
                np.nanpercentile(self.calibhisto["rms2d"], 99.5),
            )
        ...
        plt.subplot(gsPed[0]).plot(
            self.calibhisto["runs"], self.calibhisto["ped2dmax"], "bo"
        )
        plt.subplot(gsPed[0]).plot(
            self.calibhisto["runs"],
            self.calibhisto["ped2dmax"] + self.calibhisto["ped2dfwhm"],
            "ro",
        )
        plt.subplot(gsPed[0]).plot(
            self.calibhisto["runs"],
        ...
        plt.subplot(gsPed[1]).plot(
            self.calibhisto["runs"], self.calibhisto["status"], "o"
        )

        ch_ped2d = self.calibhisto["ped2d"]
        im0 = plt.subplot(gsPed[2]).imshow(
            ch_ped2d.T,
            vmin=np.nanpercentile(ch_ped2d, 1),
            vmax=np.nanpercentile(ch_ped2d, 99),
            interpolation="none",
            aspect="auto",
            extent=[
                0,
                ch_ped2d.shape[0],
                self.calibhisto["pedBinLow"],
                self.calibhisto["pedBinLow"]
                + ch_ped2d.shape[1] * self.calibhisto["pedBinWidth"],
            ],
            origin="lower",
        )
        cbar0 = plt.colorbar(im0)
        ...
        im2 = plt.subplot(gsPed[3]).imshow(
            ch_rms2d.T,
            vmin=np.nanpercentile(ch_rms2d, 1),
            vmax=np.nanpercentile(ch_rms2d, 99),
            interpolation="none",
            aspect="auto",
            extent=[
                0,
                ch_rms2d.shape[0],
                self.calibhisto["rmsBinLow"],
                self.calibhisto["rmsBinLow"]
                + ch_rms2d.shape[1] * self.calibhisto["rmsBinWidth"],
            ],
            origin="lower",
        )
        cbar2 = plt.colorbar(im2)

    def compareCommonMode(
        self,
        detname="None",
        common_modes=[],
        numEvts=100,
        ...
        if thresRms != 0:
            baseName += "thresRms%d_" % int(thresRms * 10.0)
        imgNames = []
        imgs = []
        ...
        for cm in common_modes:
            ...
            if self.__dict__[detname].ped.shape != self.__dict__[detname].imgShape:
                imgs.append(
                    self.__dict__[detname].det.image(
                        self.run, self.__dict__[imgNames[-1]]
                    )
                )
            else:
        ...
        for icm, cm in enumerate(common_modes):
            if cm == 0:
                lims = [
                    np.nanpercentile(imgs[icm * 2], 1),
                    np.nanpercentile(imgs[icm * 2], 99),
                ]
            else:
                lims = [np.nanpercentile(imgs[0], 1), np.nanpercentile(imgs[0], 99)]
            limsStd = [np.nanpercentile(imgs[1], 1), np.nanpercentile(imgs[1], 99)]
            imC = plt.subplot(gsCM[icm * 2]).imshow(
                imgs[icm * 2], clim=lims, interpolation="none", aspect="auto"
            )
            plt.colorbar(imC)
            imCS = plt.subplot(gsCM[icm * 2 + 1]).imshow(
                imgs[icm * 2 + 1], clim=limsStd, interpolation="none", aspect="auto"
            )
            plt.colorbar(imCS)
    ...
    def makeCubeData(
        ...
        if isinstance(nEvtsPerBin, str):
            nEvtsPerBin = int(nEvtsPerBin)

        # smallData only version: only run on single core
        if len(myCube.targetVarsXtc) <= 0:
            if rank == 0:
                outFileName = "/Cube_" + self.sda.fname.split("/")[-1].replace(
                    ".h5", f"_{cubeName}.h5"
                )
                ...
                printR(
                    rank, f"Now write outputfile (only small data) to : {outFileName}"
                )
        ...
        if offEventsCube > 0:
            ...
            addVars = [
                "offNbrs_event_time_xon_nNbr%02d" % offEventsCube,
        ...
        for f, t in zip(eventIdxDict["fiducial"], eventIdxDict["evttime"]):
            if nEvtsPerBin > 0:
                bins_info.append(
                    [np.asarray(f[:nEvtsPerBin]), np.asarray(t[:nEvtsPerBin])]
                )
            else:
        ...
        for detname, detDict in zip(self.detNames, myCube.targetVarsXtc):
            ...
            try:
                ...
                logger.warning(
                    "Could not make dataset for detector {}. Exit. {}".format(
                        detname, e
                    )
                )
                comm.Abort()
        ...
        print(
            f'Renaming file from {outFileName} to {outFileName.replace(".inprogress","")}, remove random variable if applicable\n\n'
        )
    ...
    def _broadcast_xtc_dets(self, cubeName):
        ...
        myCube.targetVarsXtc = [
            {"source": det, "full": 1} if isinstance(det, str) else det
            for det in targetVarsXtc
        ]
