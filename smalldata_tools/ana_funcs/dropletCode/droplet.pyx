...
cdef extern from "_droplet.c":
    ...
    void raw_dropletanal(np.int_t *img, np.int_t* dimg, np.int_t *npix,\
            np.float_t *xcen, np.float_t *ycen,\
            np.float_t *xsig, np.float_t *ysig, np.float_t *xysig,\
	    np.int_t * adus, np.int_t * idlist,\
...
def dropletanal(np.ndarray[np.int_t, ndim=2, mode="c"] img, \
    ...
    xsig : intensity (in img) weighted center of mass for x*x

    ysig : intensity (in img) weighted center of mass for y*y

    xysig: intensity (in img) weighted center of mass for x*y
 ...
 '''
    ...
    cdef np.ndarray[np.float_t, ndim=1, mode="c"] xsig = np.zeros(npeaks, dtype=np.float_)
    cdef np.ndarray[np.float_t, ndim=1, mode="c"] ysig = np.zeros(npeaks, dtype=np.float_)
    cdef np.ndarray[np.float_t, ndim=1, mode="c"] xysig = np.zeros(npeaks, dtype=np.float_)
    ...
    raw_dropletanal(&img[0,0], &dimg[0,0], &npix[0], &xcen[0], &ycen[0],\
                    &xsig[0],&ysig[0],&xysig[0],&adus[0], &idlist[0], npeaks, d[1], d[0])
