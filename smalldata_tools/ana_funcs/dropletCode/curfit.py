...
def curfit(x,y,sigy,a0,function_name=None,adj=None):
  ...
  notes:
    ...
    afit, cv, idt,m,ie = leastsq(_residuals,a[adj],\
            args=(x,y,sigy,a,adj,function_name),full_output=True,\
            ftol=.001,epsfcn=.00025**2)
...
def fitpr(chisq,a,sigmaa,title=None,lbl=None):
    ...
    str=''
    if(title is not None): str=title+' '
    str+='chisq=%(c).4f'%{'c':chisq}
    for i in range(a.size):
        str += '\n     %(lbl)8s =%(m)10.4f +/- %(s).4f'%{'lbl':lbl[i],'m':a[i],'s':sigmaa[i]}
    return(str)
#easy plot for fit
