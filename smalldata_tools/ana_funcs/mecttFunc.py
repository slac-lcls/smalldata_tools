...
class mecttFunc(DetObjectFunc):
    ...
    def __init__(self, **kwargs):
        ...
        if self.roi_n.shape[0] == 2 and len(self.roi_n.shape) == 2:
            self.bkg_n = self.background[
                self.roi_n[0, 0] : self.roi_n[0, 1], self.roi_n[1, 0] : self.roi_n[1, 1]
            ]
        else:
    ...
    def tt_ana(self, data):
        ...
        sig = tt_im_rot[
            self.roi_s[0, 0] : self.roi_s[0, 1], self.roi_s[1, 0] : self.roi_s[1, 1]
        ]
        if self._debug:
    ...
    def moments1D(self, inpData):
        ...
        bkgC = inpData[
            0
        ]  # Taking first point as intercept and fitting a straght line for slope
        bkgSlope = (inpData[-1] - inpData[0]) / (len(inpData) - 1.0)

        Data = np.ma.masked_less(
            inpData - (bkgC + bkgSlope * np.arange(len(inpData))), 0
        )
