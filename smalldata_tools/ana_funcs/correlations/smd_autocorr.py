...
class Autocorrelation(DetObjectFunc):
    ...
    def __init__(self, **kwargs):
        ...
        if "illumination_correction" in kwargs:
            self.illumination_correction = np.load(
                kwargs["illumination_correction"]["correction"], allow_pickle=True
            )
    ...
    def check_mask_save_range(self):
        ...
        if min_size // 2 < np.max(self.save_range):
            print(
                f"save_range is bigger than the autocorr array. Reducing it to ({min_size//2-1}, {min_size//2-1})"
            )
    ...
    def process(self, img):
            ...
            for ii, mask in enumerate(self.mask):
                if self.illumination_correction is not None:
                    img_box, mask = utils.box_to_roi_extend(
                        img, mask, extend=2 * self.illumination_kernel
                    )
                    img_corr = img_box / self.illumination_correction[ii]
                    autocorr.append(
                        corr.spatial_correlation_fourier(img_box, mask=mask)
                    )
                    cr.append(
                        img_box[mask].mean()
                    )  # careful if mask is not bool, this does weird things
                else:
        ...
        if self.save_lineout:
            ...
            output = {"lineout_h": lineout_h, "lineout_v": lineout_v, "cr": cr}
        else:
            output = {
                "autocorr": autocorr,
                "cr": cr,
