...
class droplet2Photons(DetObjectFunc):
    ...
    def onephoton_max(self, pixones, npix_drop, ppos):
        ...
        for drop in range(len(npix_drop)):
            ...
            nbrpix = filters.maximum_filter(
                piximg1, mode="constant", footprint=self._footprintnbr
            ).flatten()[np.argmax(piximg)]
    ...
    def onephoton(self, image, imgDrop, drop_ind, detail=False):
        ...
        """
        adu_drop = ndi.sum_labels(
            image, labels=imgDrop, index=drop_ind
        )  # only to check!
        pos_drop = np.asarray(ndi.center_of_mass(image, labels=imgDrop, index=drop_ind))
        npix_drop = ndi.sum_labels(
            image.astype(bool).astype(int), labels=imgDrop, index=drop_ind
        ).astype(int)
    ...
    def multi_photon(self, image, imgDrop, drop_ind):
        ...
        npix_drop = (
            ndi.sum_labels(
                image.astype(bool).astype(int), labels=imgDrop, index=drop_ind_thres
            )
        ).astype(
            int
        )  # need
        ...
        pos_drop = np.asarray(
            ndi.center_of_mass(image, labels=imgDrop, index=drop_ind_thres)
        )
    ...
    def process(self, data):
        ...
        if self.cputime:
            ret_dict["cputime"] = np.array(
                [time.time() - time0, timed - time0, timep - timed, time.time() - timep]
            )
