...
class fitCenter(DetObjectFunc):
    def __init__(self, **kwargs):
        ...
        userMask = kwargs.get(
            "userMask", None
        )  # fill these from detector object. Unless mask is set from userMask
        self._mask = None
        if userMask is not None:
            self._mask = np.loadtxt(userMask)
            if self._xArr is not None and self._mask.shape != self._xArr.shape:
                if (
                    self._mask.shape[1] == 2
                ):  # what is this for - seems to be turning a cs140 mask around?
    ...
    def process(self, image):
        ...
        res = fitCircle(
            self._xArr.flatten()[image.flatten() > thresP],
            self._yArr.flatten()[image.flatten() > thresP],
        )
        res.pop("msg", None)
        res.pop("info", None)
