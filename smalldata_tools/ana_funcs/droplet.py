    ...
    def dropletize(self, data):
        ...
        if self.threshold != self.thresholdLow:
            if len(img_drop[0].shape) == 2:
                ...
                imgDrop = np.array(
                    [self.neighborImg(imgd_tile) for imgd_tile in img_drop[0]]
                )
        ...
        if not self._needProps:
            # Faster option
            drop_adu = np.array(
                ndi.sum_labels(img, labels=imgDrop, index=drop_ind_thres)
            )
            pos_drop = np.array(ndi.center_of_mass(img, imgDrop, drop_ind_thres))
            npix_drop = (
                ndi.sum_labels(
                    img.astype(bool).astype(int), labels=imgDrop, index=drop_ind_thres
                )
            ).astype(int)
            ...
            for droplet, ds in zip(self.regions, dropSlices):
                ...
                if pixelArray.shape[0] > self._nMaxPixels:
                    ...
                    images.append(
                        np.append(
                            pixelArray, np.zeros(self._nMaxPixels - pixelArray.shape[0])
                        )
                    )
