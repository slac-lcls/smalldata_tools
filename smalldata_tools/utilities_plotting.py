...
def get_all_mpl_palettes(allmaps=None):
    ...
    for cmap_name in allmaps:
        ...
        palette = [
            matplotlib.colors.rgb2hex(m) for m in cmap(np.arange(cmap.N))
        ]  # always 256 pts!!!
        palettes[-1] += palette
        palette_nums[-1] += [int(m[1:], 16) for m in palette]
        map_names += [cmap_name]
    ...
    return {
        "map_names": map_names,
        "palettes_dict": palettes_dict,
        "palette_nums_dict": palette_nums_dict,
    }
...
# define cmaps.
#
cmaps = get_all_mpl_palettes(
    allmaps=["jet", "gray", "cool", "hot", "seismic", "CMRmap", "nipy_spectral"]
)
...
def create_cmap_selection(im, cmaps=None, value="jet"):
    ...
    call_cm = bokeh.models.CustomJS(
        args=dict(im=im, bmem_cmaps=bmem_cmaps),
        code="""
    ...
    select_cm = bokeh.models.Select(
        title="Color Map:", value=value, options=cmaps["map_names"], callback=call_cm
    )
...
def create_map(
    ...
    imquad = p.quad(
        top=[y1], bottom=[y0], left=[x0], right=[x1], alpha=0
    )  # This is used for hover and taptool
    if create_colorbar:
        color_bar = bokeh.models.ColorBar(
            color_mapper=im.glyph.color_mapper, label_standoff=12, location=(0, 0)
        )
...
def create_range_input_button(plotMin, plotMax, im):
    ...
    input_plotMin = bokeh.models.widgets.TextInput(
        value="%g" % plotMin, title="plotMin"
    )
    input_plotMax = bokeh.models.widgets.TextInput(
        value="%g" % plotMax, title="plotMax"
    )
    button_update = bokeh.models.Button(
        label="Update plotMin/plotMax", callback=callback_update
    )
...
def create_range_slider(vabsmin, vabsmax, plotMin, plotMax, im, step=0.1):
    ...
    try:
        callback_slider = bokeh.models.CustomJS(args=dict(im=im), code=JS_code_slider)
        rslider = bokeh.models.RangeSlider(
            title="low/high limit",
            start=vabsmin,
            end=vabsmax,
            step=step,
        ...
        callback_slider = bokeh.models.CustomJS(
            args=dict(im=im), code=JS_code_slider_b0215
        )
        rslider = bokeh.models.RangeSlider(
            title="low/high limit",
            start=vabsmin,
...
def create_img_slider_scale(im, imOrg, valStart=0, coords=None, p=None):
    ...
    callback_slider = bokeh.models.CustomJS(
        args=dict(
            im=im, p=p, source=source, sourceShp=sourceShp, sourceCoords=sourceCoords
        ),
        code=JS_code_slider,
    )

    rslider = bokeh.models.Slider(
        title="image",
        start=coords[0],
        end=coords[-1],
        step=step,
...
def plotImageBokeh(data, **kwargs):
    ...
    step = (plotMaxP - plotMinP) / 100.0
    range_slider = create_range_slider(
        plotMin, plotMax, plotMinP, plotMaxP, im=im, step=step
    )
    ...
    if rangeInput:
        ...
        layout = bp.gridplot(
            [
                [p],
                [range_slider, select_cm],
                [range_input[2], range_input[0], range_input[1]],
            ]
        )
    else:
...
def plot2d_from3d(data2plot, **kwargs):
        ...
        if init_plot is not None:
            ...
            if not isinstance(initIdx, int):
                if not isinstance(init_plot, int):
                    print(
                        "init_plot needs to be integer, using z-axis to be implemented later, will start at first image"
                    )
                    initIdx = 0
                else:
                    initIdx = init_plot
    ...
    if coord is None or (
        len(data2plot.shape) == 3 and data2plot.shape[0] != len(coord)
    ):
    ...
    imgSource = ColumnDataSource({"value": init_dat})
    # create figure.
    p = bp.figure(
        x_range=(x0, x1),
        y_range=(y0, y1),
    ...
    # range slider.
    step = (plotMaxP - plotMinP) / 50.0
    # vabsmin,vabsmax = plotMin, plotMax
    range_slider = create_range_slider(
        np.nanmin(data2plot), np.nanmax(data2plot), plotMinP, plotMaxP, im=im, step=step
    )
    ...
    if len(data2plot.shape) == 3:
        img_slider = create_img_slider_scale(
            im, data2plot, valStart=init_plot, coords=coord, p=p
        )
        # put them togeter.
        if rangeInput:
            layout = bp.gridplot(
                [
                    [p],
                    [range_slider, select_cm, img_slider],
                    [range_input[2], range_input[0], range_input[1]],
                ]
            )
        else:
...
def getColors(nColors=None, maxCol=2.6, minCol=0.25):
    ...
    # sort them
    by_hsv = sorted(
        (tuple(mcolors.rgb_to_hsv(mcolors.hex2color(color)[:3])), name)
        for name, color in colors.items()
        if (
            np.array(mcolors.hex2color(color)[:3]).sum() < maxCol
            and np.array(mcolors.hex2color(color)[:3]).sum() > minCol
        )
    )
...
def plotMarker(data, **kwargs):
    ...
    plotFilename = kwargs.pop(
        "plotFilename",
        "%s/%s_%s_histogram" % (plotDirname, runLabel, xLabel.replace("/", "_")),
    )
    ...
    if plotWith.find("matplotlib") >= 0:
        ...
        if ylim is not None:
            plt.subplot(fig).set_ylim(
                np.nanmin(np.array(ylim)), np.nanmax(np.array(ylim))
            )
        ...
        if width_height is None:
            p = bp.figure(
                title=plotTitle, x_axis_label=xLabel, y_axis_label=yLabel, tools=tools
            )
        else:
            p = bp.figure(
                title=plotTitle,
...
def plotImage(image, **kwargs):
    ...
    if len(image.shape) != 2:
        print(
            "plotImage need to be given a 2-d array, got this shape",
            image.shape,
            " instead, will quit",
        )
        return
    ...
    plotFilename = kwargs.pop(
        "plotFilename",
        "%s/%s_%s_vs_%s"
        % (plotDirname, runLabel, xLabel.replace("/", "_"), yLabel.replace("/", "_")),
    )
    ylim = kwargs.pop("ylim", None)
    width_height = kwargs.pop("width_height", None)
    ...
    if plotWith.find("matplotlib") >= 0:
        ...
        retValues = plotImageBokeh(
            image,
            plotWidth=width_height[0],
            plotHeight=width_height[1],
            palette_name="jet",
            output_quad=output_quad,
            tools=tools,
            ...
            if self.plot_dirname is not None:
                bp.output_file(
                    ...
                    % (
                        self.plot_dirname,
                        self.runLabel,
                        plotvars[0].replace("/", "_"),
                        plotvars[1].replace("/", "_"),
                    )
                )
                bp.save(layout)
...
def plot3d_img_time(data2plot, **kwargs):
    ...
    p1d = plotMarker(
        ...
        xData=bin1d,
        fig="return_me",
        plotWith="bokeh_notebook",
        width_height=(plotWidth, plotHeight),
        plotTitle="ROI vs scan",
    )
    ...
    callback = bokeh.models.CustomJS(
        args=dict(p1d=p1d, sourceShp=sourceShp, source=source),
        code="""
...
def hv_image(data2plot, **kwargs):
    ...
    if clipping is None:
        if cmap == "gray":
            clipping = {"min": "blue", "max": "magenta", "NaN": "green"}
        elif cmap == "fire":
            clipping = {"min": "blue", "max": "magenta", "NaN": "gray"}
        elif cmap == "seismic":
            clipping = {"min": "green", "max": "magenta", "NaN": "gray"}
        elif cmap == "viridis":
            clipping = {"min": "blue", "max": "magenta", "NaN": "gray"}
        elif cmap == "jet":
            clipping = {"min": "black", "max": "magenta", "NaN": "gray"}
        elif cmap == "nipy_spectral":
            clipping = {"min": "black", "max": "magenta", "NaN": "gray"}
        elif cmap == "rainbow":
            clipping = {"min": "black", "max": "magenta", "NaN": "white"}
        else:
            clipping = {"min": "black", "max": "magenta", "NaN": "white"}
    ...
    if plotLog:
        ...
        data2plot_log[np.isneginf(data2plot_log)] = (
            0.0  # set negative vaues to zero (likely noise....)
        )
    ...
    image = hv.Image(data2plot, **imkwargs).options(
        colorbar=True, width=width, height=height, cmap=cmap, clipping_colors=clipping
    )
    return image
...
def _hv_image(data2plot, cmap, imgLow, imgHigh, **kwargs):
    ...
    if data2plot.ndim == 2:
        ...
        if bounds is not None:
            imkwargs["bounds"] = bounds
        images = [
            hv.Image(data2p, **imkwargs).options(
                colorbar=True,
                width=width,
                height=height,
                cmap=cmap,
...
def hv_imageFromIndex(index, cmap, imgLow, imgHigh, data2plot, **kwargs):
    ...
    return hv_image(
        data2plot_2d, cmap=cmap, imgLow=imgLow, imgHigh=imgHigh, kwargs=kwargs
    )
...
def hv_3dimage(data2plot, **kwargs):
    ...
    if imgPHigh > valuesLow[-1]:
        valuesLow.append(imgPHigh)
