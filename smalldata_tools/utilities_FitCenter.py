...
def _transform_hough_array(
    ar_hough, radii, zip_obj, center_x, center_y, dr, r_low, r_hi
):
...
def iterate_center(
    sparse_edges, overfill, r_range, r_bin, c_bin, prec, red_factor, norm, min_dr
):
    ...
    """
    x_range = [
        sparse_edges.shape[0] * (1.0 - overfill),
        sparse_edges.shape[0] * overfill,
    ]
    y_range = [
        sparse_edges.shape[1] * (1.0 - overfill),
        sparse_edges.shape[1] * overfill,
    ]
...
def _calc_r_vals(radii, maxdim_r, norm, min_dr):
    ...
    res_at_radii, max_radii = (
        list(x) for x in zip(*sorted(zip(res_at_radii, max_radii)))
    )
...
def _fit_circles(x, y, r, yerr=False, guess=None):
    ...
    fit_res = {}
    if yerr:
        center, C, info, msg, success = optimize.leastsq(
            f_global, center_estimate, args=(r, x, y), full_output=True
        )
        fit_res["C"] = C
        fit_res["info"] = info
        fit_res["msg"] = msg
...
def ransac_result(
    ...
    for ir, r in enumerate(r_vals):
        ...
        model, inliers = ransac(
            np.array(
                [sparse_edges.row[list(in_ring)], sparse_edges.col[list(in_ring)]]
            ).T,
            CircleModel,
        ...
        if (
            cur_ring_info["in_frac"] > min_frac
            and inliers.astype(int).sum() >= min_points
        ):
...
def FindFitCenter(image, mask, **kwargs):
    ...
    r_vals, x, y = iterate_center(
        sparse_edges, overfill, r_range, r_bin, c_bin, prec, red_factor, norm, min_dr
    )
    res, ring_info, edges = ransac_result(
        sparse_edges,
        [x, y],
        r_vals,
        n_max,
...
def FindFitCenterPlot(image, mask, **kwargs):
    ...
    plt.imshow(
        ...
        interpolation="none",
        cmap="gray",
        clim=[0, np.nanpercentile(image.flatten(), 99.5)],
    )
    ...
    for ir, thisRingInfo, rFit in itertools.izip(
        itertools.count(), ringInfo, combRes["R"]
    ):
