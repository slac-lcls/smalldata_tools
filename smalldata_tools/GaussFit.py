...
def GaussFit(data, xaxis=None, debug=False):
    ...
    if params_estimate[1] == 0:
        print(
            "params_estimate failed, will return nans.",
            params_estimate[0],
            " ",
            params_estimate[1],
        )
    ...
    try:
        ...
        try:
            ...
            fit_params_error = [-1.0, -1.0, -1.0, -1.0]
    except:
        if debug:
            print("curve fit failed failed")
        fit_params = [-1.0, -1.0, -1.0, -1.0]
        fit_params_error = [-1.0, -1.0, -1.0, -1.0]

    fit_results = {
        "mean": float(fit_params[0]),
...
def fitPeaks(peakDat, peakWindow, means, sigmas, heights, showPlot=False):
    ...
    while highEdge < peakDat.size and lowEdge < highEdge - peakWindow[0] * 0.25:
        ...
        if fitAr is not None:
            ...
            # if res is close to a previous one, add 1/2 window and redo
            elif len(means) > 0 and np.abs((fitAr["mean"] + lowEdge) - means[-1]) < (
                2 * fitAr["sigma"]
            ):
                lowEdge += peakWindow[0] * 0.5
                highEdge = min(peakDat.size, lowEdge + peakWindow[0])
                ...
                if showPlot:
                    plt.plot(
                        [fitAr["mean"], fitAr["mean"]],
                        [
                            fitAr["pedestal"] * 0.9,
                            (fitAr["height"] + fitAr["pedestal"]) * 1.05,
                        ],
                        "r",
                    )
                if debug:
                ...
                if (fitAr["mean"] + 2 * fitAr["sigma"]) < (
                    peakWindow[0] - peakWindow[1]
                ):
...
if __name__ == "__main__":
    ...
    params = np.empty(4)
    data, params[0], params[1], params[2], params[3] = create_test_gaussian(
        xaxis, noise_amplitude
    )
